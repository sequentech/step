...
sed -i.bak "s|^KC_TRUSTEE_CLIENT_SECRET=.*|KC_TRUSTEE_CLIENT_SECRET=$KC_TRUSTEE_CLIENT_SECRET|g" "$ENV_FILE" && rm "$ENV_FILE.bak"
jq --arg secret "$KC_TRUSTEE_CLIENT_SECRET" '(.clients[] | select(.clientId == "native-trustee") | .secret) = $secret' "$KEYCLOAK_JSON_FILE" > "$KEYCLOAK_JSON_FILE.tmp" && mv "$KEYCLOAK_JSON_FILE.tmp" "$KEYCLOAK_JSON_FILE"
