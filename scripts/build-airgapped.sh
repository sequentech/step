...
add-up-script-to-tarball() {
    tmpdir=$(mktemp -d)
    cat <<'EOF' > $tmpdir/docker-compose.yml
# SPDX-FileCopyrightText: 2023-2024 Eduardo Robles <edu@sequentech.io>
#
# SPDX-License-Identifier: AGPL-3.0-only
...
services:
  devcontainer:
    profiles: ["full", "base"]
    image: ghcr.io/cachix/devenv:v1.0.7
    pull_policy: never
  ...
  windmill:
    ...
    entrypoint: [
      ...
      "rust-local-target/",
      "-x",
      "run --bin main consume -q short_queue tally_queue reports_queue beat communication_queue import_export_queue"
    ]
    volumes_from:
      - devcontainer
    depends_on:
