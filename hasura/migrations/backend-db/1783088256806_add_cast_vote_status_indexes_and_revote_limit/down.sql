-- Reverse of up.sql, in reverse order. The revote limit trigger is restored to
-- its status-agnostic version first, before the `status` column is dropped.
...
DROP INDEX IF EXISTS sequent_backend.idx_cast_vote_in_progress;
