...
### Datafix voter-list reconciliation
...
an agreed Datafix freeze window:

1. Upload the complete Datafix reconciliation CSV. The first line must be the
   `#META` line and file channels must be uppercase.
2. Review the Datafix and Sequent tables. If the Datafix table is non-empty,
   download its patch, have Datafix apply it, and upload the newly generated
   reconciliation file. Never apply the Sequent side first.
3. When the Datafix table is empty, review the category totals and explicitly
   apply the Sequent changes.
...
row failure. During a hard-down Datafix freeze it cannot resolve because the
review beat must reach Datafix. If all remaining failures have this cause,
complete the freeze hash checks, restore Datafix connectivity, wait for the
review beat to resolve the ballots, and retry the same `Sequence`. Apply-time
