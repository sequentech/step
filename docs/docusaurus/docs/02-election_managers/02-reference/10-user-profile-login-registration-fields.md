...
| `html-attribute:<name>` | Sets an arbitrary HTML attribute, e.g. `html-attribute:autocomplete` = `off`. On the attribute-based login page it overrides the theme's own `tabindex` and `autocomplete`. |
