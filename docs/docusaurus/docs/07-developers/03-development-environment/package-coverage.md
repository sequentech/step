...
- `excluded_files` names exact support/test files with reasons. Their covered and
  total counters, including raw function records, are removed from every export.
  Base and head use the same exclusions; excluded tests still run.
