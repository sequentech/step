...
1. **Stage 1 — Provisioning** (`step-cli`, network calls to Hasura/Keycloak
   ...
   `packages/step-cli/scripts/setup-telephone-load-test.sh`, invoked with
   `--voting-channel ONLINE`. It imports an election event, bulk-creates
   voters, runs the keys ceremony, publishes, and opens **ONLINE** voting.
   Ends with an election event ready to accept portal votes, plus a voters
   CSV and a `summary.json` recording the portal login URL.
2. **Stage 2 — Casting** (`Playwright`): renders a voter manifest from the
   ...
   Implemented as `packages/step-cli/scripts/run-online-load-test.sh` plus
...
opened at the end differs, so `setup-telephone-load-test.sh` grew a
`--voting-channel ONLINE|TELEPHONE` enum arg (default `TELEPHONE`, preserving
its original behavior) rather than a second script. The DTMF-safe numeric
credentials it generates are a telephone constraint but work fine in the
portal login form, so voter generation stays a single code path. For `ONLINE`
the script records `voting_channel`, `voting_portal_url` and `login_url`
(`{portal}/tenant/{tenant_id}/event/{election_event_id}/login`) in
`summary.json`; both Stage 2 runners check `voting_channel` and refuse a run
dir provisioned for the other channel, since each channel's eligibility check
gates on its own channel being open.
...
(`--voter-area-name`, defaulting to the election event's first area), so
...
The telephone Stage 2 fans out with `xargs -P` because each call is a cheap
independent CLI process. Browsers are not cheap: N separate `playwright test`
invocations would each pay a Node + browser startup and multiply memory.
`run-online-load-test.sh` therefore invokes Playwright **once** with
`--workers <concurrency>`; each worker reuses one browser process, and every
voter runs in a fresh browser context inside it (so sessions don't leak
between voters, and no explicit logout is needed). The runner's
`--concurrency` arg keeps the same CLI surface as the telephone script — the
...
`VOTER_USERNAME`/`VOTER_PASSWORD` env vars and runs a single voter — the
smoke-test and selector-debugging mode.
...
- **CPU**: login and GraphQL are I/O-bound, but ballot encryption is a real
  ...
  compose stack.
- **Defaults**: `--concurrency 4`; rule of thumb
...
- **Don't load-test webpack-dev-server**: `yarn start:voting-portal` is a
  ...
  before the backend does. For bigger runs point `--voting-portal-url` at a
  production build served statically, or at a deployed portal.
...
Stage 2 (both `run-online-load-test.sh` and `run-telephone-load-test.sh`) is
...
files. The event itself lives in Keycloak/Hasura, not in the run dir. What
breaks when Stage 2 moves to another machine, and the fixes:
...
1. **Endpoint URLs inside `summary.json` are whatever Stage 1 used.** In the
   ...
   - *Preferred*: provision Stage 1 with the **public** URLs
     (`--endpoint-url`, `--keycloak-url`, `--voting-portal-url`), so
     `summary.json` is remote-valid as written — distribution is then purely
     file copying.
   - *Also*: both Stage 2 runners accept `--keycloak-url` / `--hasura-url`
     overrides (and the online runner `--voting-portal-url`), so an existing
     locally-provisioned run dir can still be replayed remotely.
2. **Getting the run dir there**: `scp`/`rsync` the Stage-1 `--out-dir` (plus
   the DTMF template for telephone). Both runners tolerate a moved dir — the
   CSV path in `summary.json` falls back to `<run-dir>/<basename>`.
3. **Disjoint voter shards per machine — the one correctness invariant.**
   ...
   skewing results). `--voter-offset` + `--max-votes`/`--max-calls` slice the
   CSV per machine: machine A `--voter-offset 0 --max-votes 500`, machine B
   `--voter-offset 500 --max-votes 500`, and so on. Provision Stage 1 with
   `--num-voters` ≥ machines × per-machine votes. (The telephone runner
   derives each call's synthetic caller number from the voter id, so
   disjoint voter slices also keep caller numbers unique across machines.)
4. **Telephone-only machine-local dependencies**: each load machine needs the
   ...
   network, so remotely each machine runs its own local valkey/redis and
   passes `--valkey-url`. Nothing needs sharing between machines: IVR
