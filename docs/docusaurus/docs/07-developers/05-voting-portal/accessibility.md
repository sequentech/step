...
`<button>`. `buttonClassName` also targets the button; `className` continues to target
the SVG for compatibility. Other icon props are spread onto the icon, where an
