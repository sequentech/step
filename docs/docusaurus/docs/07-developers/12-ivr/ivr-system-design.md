...
- **API Integration**: Direct integration with existing Harvest API endpoints
...
flowchart LR
    ...
    C --> G[Hasura/<br/>GraphQL]
    C --> H[Harvest API<br/>Voting]
```
...
| **Public S3** | Published election config: IVR auth steps, prompts, election structure (same bucket used by ballot publication) |
| **Keycloak** | Voter authentication via OIDC Direct Grant (ROPC) with configurable auth factors, JWT issuance |
| **Hasura/GraphQL** | Fetch elections, ballots, candidates (authenticated, post-login) |
| **Harvest API** | Cast votes via Hasura GraphQL Action |
...
### 3.2 Multi-Digit DTMF Input Handling
...
### 3.4 Flow Engine Implementation
...
All IVR config (flow phases, auth steps, prompts) is published to the **public S3 bucket** as part of the existing ballot publication workflow — the same mechanism used for ballot styles, election event data, and support materials (see `upload_and_return_document()` in `packages/windmill/src/services/documents.rs` with `is_public: true`).
...
3. Ballot publication task publishes election data to public S3, including IVR config
4. Published data is publicly accessible — no authentication needed

**Lambda session initialization:**
1. Call arrives → Lambda reads DynamoDB `ivr-phone-config` → gets cluster + environment URLs + tenant_id + election_event_id
2. Lambda fetches published election config from public S3 (flow, auth steps, prompts, election structure)
3. Config cached in DynamoDB session for the call duration
...
### 5.1.1 Token Expiry Handling (Critical)
...
For operations that are NOT vote submission (e.g., fetching elections), we can be more lenient:

```rust
async fn fetch_elections(&self, session: &IvrSession) -> Result<Vec<Election>, IvrError> {
    ...
    // Try the API call
    match self.hasura_client.get_elections(token, &session.election_event_id).await {
...
### 5.2 Fetch Elections via Hasura GraphQL

**Important:** IVR Lambda calls **Hasura GraphQL** endpoint, not Harvest directly.
...
sequenceDiagram
    ...
    Lambda->>Hasura: POST /v1/graphql<br/>Authorization: Bearer {jwt}<br/>Query: GetOpenElections
    Hasura-->>Lambda: { elections: [...] }
```
...
**GraphQL Query**:
```graphql
query GetOpenElections($eventId: uuid!) {
  sequent_backend_election(
    where: {
      election_event_id: { _eq: $eventId },
      status: { _contains: { voting_status: "OPEN" } },
      voting_channels: { _contains: { telephone: true } }
    }
  ) {
    id
    name
...
**Note:** Filter uses `telephone: true` in `voting_channels` to only return phone-enabled elections.

### 5.3 Cast Vote via Hasura GraphQL Action

**Important:** IVR Lambda calls **Hasura GraphQL Action** `insert_cast_vote`, which then routes to Harvest (see `hasura/metadata/actions.yaml`).
...
sequenceDiagram
    ...
    Lambda->>Hasura: POST /v1/graphql<br/>Mutation: insert_cast_vote
    Hasura->>Harvest: POST /insert-cast-vote
    Harvest-->>Hasura: { cast_vote_id }
    Hasura-->>Lambda: { cast_vote_id }
```
...
**GraphQL Mutation**:
```graphql
mutation InsertCastVote($input: InsertCastVoteInput!) {
  insert_cast_vote(input: $input) {
    cast_vote_id
    status
  }
}
...
- Hasura forwards JWT to Harvest, which extracts `area_id` from claims
...
- Lambda deployed in VPC with access to Keycloak and Harvest API
- Lambda IP whitelisted in Keycloak and Hasura (as noted in CTO notes)
...
2. **Blacklist Storage**: Where should phone number blacklists be stored?
   ...
   - Option B: PostgreSQL via Hasura (consistent with other data, but requires auth)
