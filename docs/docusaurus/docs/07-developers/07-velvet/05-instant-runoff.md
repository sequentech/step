...
In multi-contest encoding, IRV is supported only when the contest-level Duplicate Rank Policy is disabled (i.e., voters are not allowed to proceed with duplicate rankings).
