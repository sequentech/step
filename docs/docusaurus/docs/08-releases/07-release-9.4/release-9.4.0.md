...
To display the timezone correctly in the default `ballot_receipt_user.hbs` template, update the timestamp rendering:
...
For developers upgrading to `9.4.0`:

1. **Rust version**: use Rust `1.90.0`. Run `rustc --version` in `devenv shell` to verify.
...
cargo build
```
3. **Frontend updates**: for frontend work, run:
```bash
cd packages/
yarn install
```
4. **ESLint migration**: projects now use flat config via `eslint.config.js`. Legacy `.eslintrc.json` files have been removed.
5. **New documentation**: review:
- `docs/docusaurus/docs/07-developers/11-updates/updating-rust-version.md`
- `docs/docusaurus/docs/05-reference/third_party_deps/third_party_deps.md`
- `docs/docusaurus/docs/07-developers/03-development-environment/nvd-api-key-setup.md`
...
Release `9.4.0` expands election capabilities with support for [Instant-runoff Voting (IRV/RCV)](https://github.com/sequentech/meta/issues/8214), [delegate voting with imports](https://github.com/sequentech/meta/issues/7683), and [tenant-level tasks in the Admin Portal](https://github.com/sequentech/meta/issues/2348). It also adds [IdP-initiated SAML SSO authentication](https://github.com/sequentech/meta/issues/8213) and moves [voter signature capture into the Voting Portal](https://github.com/sequentech/meta/issues/5518).

### 🛠 Platform and developer updates
This release standardizes the repository on [Rust `1.90.0`](https://github.com/sequentech/meta/issues/7996), improves release automation through the [generalized `release-tool`](https://github.com/sequentech/meta/issues/9230) and [automatic E2E test execution](https://github.com/sequentech/meta/issues/7004), and continues the [public open-source preparation work](https://github.com/sequentech/meta/issues/9060). It also adds new documentation, including the [Docusaurus GraphQL/Hasura tutorial](https://github.com/sequentech/meta/issues/10959) and the [AI agent documentation structure](https://github.com/sequentech/meta/issues/10577).

### 🛡 Security updates
[Dependabot and related security updates](https://github.com/sequentech/step/security/dependabot) were applied across maintained branches as part of this release cycle. See [sequentech/meta#10222](https://github.com/sequentech/meta/issues/10222) for details.
