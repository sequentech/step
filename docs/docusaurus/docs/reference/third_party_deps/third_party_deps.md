...
| bb8-postgres | 0.8.1 | MIT | Full-featured async (tokio-based) postgres connection pool (like r2d2) |
...
| config | 0.13 | MIT OR Apache-2.0 | Layered configuration system for Rust applications. |
...
| hex | 0.4.2 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| log | 0.4.17 | MIT OR Apache-2.0 | A lightweight logging facade for Rust |
...
| serde | 1.0.152 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| strum | 0.26.3 | MIT | Helpful macros for working with enums and strings |
| tokio | 1.38.1 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
| tokio-postgres | 0.7.10 | MIT OR Apache-2.0 | A native, asynchronous PostgreSQL client |
| toml | 0.6.0 | MIT OR Apache-2.0 | A native Rust encoder and decoder of TOML-formatted files and streams. Provides implementations of the standard Serialize/Deserialize traits for TOML data to facilitate deserializing and serializing Rust structures. |
| tonic | 0.12 | MIT | A gRPC over HTTP/2 implementation focused on high performance, interoperability, and flexibility. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| tracing-attributes | 0.1.23 | MIT | Procedural macro attributes for automatically instrumenting functions. |
| tracing-log | 0.1.3 | MIT | Provides compatibility between `tracing` and the `log` crate. |
| tracing-subscriber | 0.3.16 | MIT | Utilities for implementing and composing `tracing` subscribers. |
| tracing-tree | 0.2.1 | MIT OR Apache-2.0 | A Tracing Layer which prints a tree of spans and events. |
...
|------------|---------|---------|-------------|
| anyhow | 1.0 | MIT OR Apache-2.0 | Flexible concrete Error type built on std::error::Error |
| ascii_table | 4.0.2 | MIT | Print ASCII tables to the terminal |
| async-trait | 0.1.73 | MIT OR Apache-2.0 | Type erasure for async trait methods |
| base64 | 0.22.1 | MIT OR Apache-2.0 | encodes and decodes base64 as bytes or utf8 |
| cfg-if | 1.0 | MIT OR Apache-2.0 | A macro to ergonomically define an item depending on a large number of #[cfg] parameters. Structured like an if-else chain, the first matching branch is the item that gets emitted. |
| clap | 4.0 | MIT OR Apache-2.0 | A simple to use, efficient, and full-featured Command Line Argument Parser |
| colored | 2.0.4 | MPL-2.0 | The most simple way to add colors in your terminal |
| crepe | 0.1.5 | MIT OR Apache-2.0 | Datalog in Rust as a procedural macro |
...
| rayon | 1.5 | MIT OR Apache-2.0 | Simple work-stealing parallelism for Rust |
| reedline-repl-rs | 1.0.7 | MIT | Library to generate a fancy REPL for your application based on reedline and clap |
| rusqlite | 0.32.1 | MIT | Ergonomic wrapper for SQLite |
| rustc-hash | 2.0.0 | Apache-2.0 OR MIT | A speedy, non-cryptographic hashing algorithm used by rustc |
| serde | 1.0.152 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| serde_json | 1.0.105 | MIT OR Apache-2.0 | A JSON serialization file format |
| strum | 0.26.3 | MIT | Helpful macros for working with enums and strings |
| thiserror | 1.0.50 | MIT OR Apache-2.0 | derive(Error) |
| tikv-jemalloc-ctl | 0.6.0 | MIT/Apache-2.0 | A safe wrapper over jemalloc's control and introspection APIs |
| tikv-jemallocator | 0.6.0 | MIT/Apache-2.0 | A Rust allocator backed by jemalloc |
| tokio | 1.40.0 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
| toml | 0.6.0 | MIT OR Apache-2.0 | A native Rust encoder and decoder of TOML-formatted files and streams. Provides implementations of the standard Serialize/Deserialize traits for TOML data to facilitate deserializing and serializing Rust structures. |
| tonic | 0.12 | MIT | A gRPC over HTTP/2 implementation focused on high performance, interoperability, and flexibility. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| tracing-attributes | 0.1.23 | MIT | Procedural macro attributes for automatically instrumenting functions. |
...
| tracing-subscriber | 0.3.16 | MIT | Utilities for implementing and composing `tracing` subscribers. |
| tracing-tree | 0.2.1 | MIT OR Apache-2.0 | A Tracing Layer which prints a tree of spans and events. |
| tracing-wasm | 0.2.1 | MIT OR Apache-2.0 | tracing subscriber for browser WASM |
| wasm-bindgen | =0.2.100 | MIT OR Apache-2.0 | Easy support for interacting between JS and Rust. |
| wasm-bindgen-rayon | 1.0.3 | Apache-2.0 | Adapter for using Rayon-based concurrency on the Web |
...
| rand | 0.8.5 | MIT OR Apache-2.0 | Random number generators and other randomness functionality. |
| reqwest | 0.12.5 | MIT OR Apache-2.0 | higher level HTTP client library |
| rocket | 0.5.0 | MIT OR Apache-2.0 | Web framework with a focus on usability, security, extensibility, and speed. |
...
| uuid | 1.5.0 | Apache-2.0 OR MIT | A library to generate and parse UUIDs. |
...
| Dependency | Version | License | Description |
|------------|---------|---------|-------------|
| anyhow | 1.0.75 | MIT OR Apache-2.0 | Flexible concrete Error type built on std::error::Error |
| borsh | 1.5 | MIT OR Apache-2.0 | Binary Object Representation Serializer for Hashing |
| clap | 4.0 | MIT OR Apache-2.0 | A simple to use, efficient, and full-featured Command Line Argument Parser |
| hex | 0.4.3 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| log | 0.4.17 | MIT OR Apache-2.0 | A lightweight logging facade for Rust |
| serde | 1.0.190 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| serde_json | 1.0 | MIT OR Apache-2.0 | A JSON serialization file format |
| strum | 0.25 | MIT | Helpful macros for working with enums and strings |
| strum_macros | 0.25 | MIT | Helpful macros for working with enums and strings |
| tokio | 1.38.1 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
| tokio-stream | 0.1.17 | MIT | Utilities to work with `Stream` and `tokio`. |
| tonic | 0.12 | MIT | A gRPC over HTTP/2 implementation focused on high performance, interoperability, and flexibility. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| tracing-attributes | 0.1.23 | MIT | Procedural macro attributes for automatically instrumenting functions. |
| tracing-log | 0.1.3 | MIT | Provides compatibility between `tracing` and the `log` crate. |
...
|------------|---------|---------|-------------|
| anyhow | 1.0.75 | MIT OR Apache-2.0 | Flexible concrete Error type built on std::error::Error |
| aws-config | 1.0.3 | Apache-2.0 | AWS SDK config and credential provider implementations. |
| aws-sdk-s3 | 1.11.0 | Apache-2.0 | AWS SDK for Amazon Simple Storage Service |
| base64 | 0.21.0 | MIT OR Apache-2.0 | encodes and decodes base64 as bytes or utf8 |
| bstr | 1.11.3 | MIT OR Apache-2.0 | A string type that is not required to be valid UTF-8. |
| celery | 0.5.5 | Apache-2.0 | Rust implementation of Celery |
| chrono | 0.4.31 | MIT OR Apache-2.0 | Date and time library for Rust |
| deadpool-postgres | 0.11.0 | MIT OR Apache-2.0 | Dead simple async pool for tokio-postgres |
| dotenv | 0.15.0 | MIT | A `dotenv` implementation for Rust |
| either | 1.9.0 | MIT OR Apache-2.0 | The enum `Either` with variants `Left` and `Right` is a general purpose sum type with two cases. |
| graphql_client | 0.13.0 | Apache-2.0 OR MIT | Typed GraphQL requests and responses |
| handlebars | 6.1.0 | MIT | Handlebars templating implemented in Rust. |
| regex | 1.10.2 | MIT OR Apache-2.0 | An implementation of regular expressions for Rust. This implementation uses finite automata and guarantees linear time matching on all inputs. |
| reqwest | 0.12 | MIT OR Apache-2.0 | higher level HTTP client library |
| rocket | 0.5.0 | MIT OR Apache-2.0 | Web framework with a focus on usability, security, extensibility, and speed. |
| serde | 1.0.190 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| serde_json | 1.0.105 | MIT OR Apache-2.0 | A JSON serialization file format |
| serde_with | 3.4.0 | MIT OR Apache-2.0 | Custom de/serialization functions for Rust's serde |
| strum | 0.25 | MIT | Helpful macros for working with enums and strings |
| strum_macros | 0.25 | MIT | Helpful macros for working with enums and strings |
| tempfile | 3.15.0 | MIT OR Apache-2.0 | A library for managing temporary files and directories. |
| tokio | 1.32.0 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
...
| hex | 0.4.3 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| log | 0.4.17 | MIT OR Apache-2.0 | A lightweight logging facade for Rust |
| tokio | 1.31.0 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
| tonic | 0.12 | MIT | A gRPC over HTTP/2 implementation focused on high performance, interoperability, and flexibility. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| tracing-attributes | 0.1.23 | MIT | Procedural macro attributes for automatically instrumenting functions. |
| tracing-log | 0.1.3 | MIT | Provides compatibility between `tracing` and the `log` crate. |
...
| anyhow | 1.0 | MIT OR Apache-2.0 | Flexible concrete Error type built on std::error::Error |
| log | 0.4.17 | MIT OR Apache-2.0 | A lightweight logging facade for Rust |
| prost | 0.13.5 | Apache-2.0 | A Protocol Buffers implementation for the Rust Language. |
| prost-types | 0.13.5 | Apache-2.0 | Prost definitions of Protocol Buffers well known types. |
| serde | 1.0.152 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| tonic | 0.12.3 | MIT | A gRPC over HTTP/2 implementation focused on high performance, interoperability, and flexibility. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| tracing-attributes | 0.1.23 | MIT | Procedural macro attributes for automatically instrumenting functions. |
| tracing-log | 0.1.3 | MIT | Provides compatibility between `tracing` and the `log` crate. |
| tracing-subscriber | 0.3.16 | MIT | Utilities for implementing and composing `tracing` subscribers. |
...
| Dependency | Version | License | Description |
|------------|---------|---------|-------------|
| ammonia | 3.3.0 | MIT OR Apache-2.0 | HTML Sanitization |
| anyhow | 1.0.75 | MIT OR Apache-2.0 | Flexible concrete Error type built on std::error::Error |
| async-trait | 0.1 | MIT OR Apache-2.0 | Type erasure for async trait methods |
| aws-config | 1.6.0 | Apache-2.0 | AWS SDK config and credential provider implementations. |
| aws-sdk-s3 | 1.79.0 | Apache-2.0 | AWS SDK for Amazon Simple Storage Service |
| aws-sdk-sesv2 | 1.70.0 | Apache-2.0 | AWS SDK for Amazon Simple Email Service |
| aws-sdk-sns | 1.63.0 | Apache-2.0 | AWS SDK for Amazon Simple Notification Service |
| aws-smithy-types | 1.3.0 | Apache-2.0 | Types for smithy-rs codegen. |
| base64 | 0.21.0 | MIT OR Apache-2.0 | encodes and decodes base64 as bytes or utf8 |
| borsh | 1.5 | MIT OR Apache-2.0 | Binary Object Representation Serializer for Hashing |
| cfg-if | 1.0 | MIT OR Apache-2.0 | A macro to ergonomically define an item depending on a large number of #[cfg] parameters. Structured like an if-else chain, the first matching branch is the item that gets emitted. |
| chrono | 0.4 | MIT OR Apache-2.0 | Date and time library for Rust |
| console_error_panic_hook | 0.1.7 | Apache-2.0/MIT | A panic hook for `wasm32-unknown-unknown` that logs panics to `console.error` |
| csv | 1.3.0 | Unlicense/MIT | Fast CSV parsing with support for serde. |
| curve25519-dalek | =4.1.2 | BSD-3-Clause | A pure-Rust implementation of group operations on ristretto255 and Curve25519 |
| ed25519-dalek | =2.1.1 | BSD-3-Clause | Fast and efficient ed25519 EdDSA key generations, signing, and verification in pure Rust. |
| handlebars | 6.1.0 | MIT | Handlebars templating implemented in Rust. |
| handlebars-chrono | 0.2.1 | BSD-2-Clause | Handlebars helper for using chrono DateTime |
| headless_chrome | 1.0.15 | MIT | Control Chrome programmatically |
| hex | 0.4 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| jsonwebtoken | 9.1.0 | MIT | Create and decode JWTs in a strongly typed way. |
| keycloak | 24.0.301 | Unlicense OR MIT | Keycloak Admin REST API. |
| kuchiki | 0.8.1 | MIT | (朽木) HTML/XML tree manipulation library |
| num-bigint | 0.4 | MIT/Apache-2.0 | Big integer implementation for Rust |
| num-format | 0.4.4 | MIT/Apache-2.0 | A Rust crate for producing string-representations of numbers, formatted according to international standards |
| num-traits | 0.2 | MIT OR Apache-2.0 | Numeric traits for generic mathematics |
| openid | 0.12.1 | Unlicense OR MIT | OpenID Connect & Discovery client library using async / await. |
...
| printpdf | 0.7.0 | MIT | Rust library for reading and writing PDF files |
| quick-error | 2.0.0 | MIT/Apache-2.0 | A macro which makes error types pleasant to write. |
| rand | 0.8 | MIT OR Apache-2.0 | Random number generators and other randomness functionality. |
| regex | 1.10.2 | MIT OR Apache-2.0 | An implementation of regular expressions for Rust. This implementation uses finite automata and guarantees linear time matching on all inputs. |
| reqwest | 0.12 | MIT OR Apache-2.0 | higher level HTTP client library |
| reqwest-middleware | =0.3.1 | MIT OR Apache-2.0 | Wrapper around reqwest to allow for client middleware chains. |
| reqwest-retry | 0.5.0 | MIT OR Apache-2.0 | Retry middleware for reqwest. |
| rocket | 0.5.0 | MIT OR Apache-2.0 | Web framework with a focus on usability, security, extensibility, and speed. |
| rusqlite | 0.32 | MIT | Ergonomic wrapper for SQLite |
| schemars | 0.8 | MIT | Generate JSON Schemas from Rust code |
| serde | 1.0.190 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| serde-wasm-bindgen | 0.4 | MIT | Native Serde adapter for wasm-bindgen |
| serde_json | 1.0 | MIT OR Apache-2.0 | A JSON serialization file format |
| serde_path_to_error | 0.1 | MIT OR Apache-2.0 | Path to the element that failed to deserialize |
| serde_urlencoded | 0.7.1 | MIT/Apache-2.0 | `x-www-form-urlencoded` meets Serde |
| sha256 | 1.6 | MIT OR Apache-2.0 | sha256 crypto digest |
| strum | 0.25 | MIT | Helpful macros for working with enums and strings |
| strum_macros | 0.25 | MIT | Helpful macros for working with enums and strings |
| tempfile | 3.15.0 | MIT OR Apache-2.0 | A library for managing temporary files and directories. |
| time | 0.3.31 | MIT OR Apache-2.0 | Date and time library. Fully interoperable with the standard library. Mostly compatible with #![no_std]. |
...
| wasm-bindgen | =0.2.100 | MIT OR Apache-2.0 | Easy support for interacting between JS and Rust. |
| web-sys | 0.3.4 | MIT OR Apache-2.0 | Bindings for all Web APIs, a procedurally generated crate from WebIDL |
...
| base64 | 0.21.0 | MIT OR Apache-2.0 | encodes and decodes base64 as bytes or utf8 |
| chrono | 0.4 | MIT OR Apache-2.0 | Date and time library for Rust |
| clap | 4.5.8 | MIT OR Apache-2.0 | A simple to use, efficient, and full-featured Command Line Argument Parser |
| csv | 1.1 | Unlicense/MIT | Fast CSV parsing with support for serde. |
| deadpool-postgres | 0.11.0 | MIT OR Apache-2.0 | Dead simple async pool for tokio-postgres |
...
| borsh | 1.5 | MIT OR Apache-2.0 | Binary Object Representation Serializer for Hashing |
| cfg-if | 1.0 | MIT OR Apache-2.0 | A macro to ergonomically define an item depending on a large number of #[cfg] parameters. Structured like an if-else chain, the first matching branch is the item that gets emitted. |
| chacha20poly1305 | 0.10.1 | Apache-2.0 OR MIT | Pure Rust implementation of the ChaCha20Poly1305 Authenticated Encryption with Additional Data Cipher (RFC 8439) with optional architecture-specific hardware acceleration. Also contains implementations of the XChaCha20Poly1305 extended nonce variant of ChaCha20Poly1305, and the reduced-round ChaCha8Poly1305 and ChaCha12Poly1305 lightweight variants. |
| curve25519-dalek | =4.1.2 | BSD-3-Clause | A pure-Rust implementation of group operations on ristretto255 and Curve25519 |
| ecdsa | 0.16.8 | Apache-2.0 OR MIT | Pure Rust implementation of the Elliptic Curve Digital Signature Algorithm (ECDSA) as specified in FIPS 186-4 (Digital Signature Standard), providing RFC6979 deterministic signatures as well as support for added entropy |
| ed25519-dalek | =2.1.1 | BSD-3-Clause | Fast and efficient ed25519 EdDSA key generations, signing, and verification in pure Rust. |
| generic-array | 0.14.6 | MIT | Generic types implementing functionality of arrays |
| getrandom | 0.2 | MIT OR Apache-2.0 | A small cross-platform library for retrieving random data from system source |
| hex | 0.4.2 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| malachite | 0.3.2 | LGPL-3.0-only | Arbitrary-precision arithmetic, with efficient algorithms partially derived from GMP, FLINT, and MPFR. |
| num-bigint | 0.4 | MIT/Apache-2.0 | Big integer implementation for Rust |
...
| num-traits | 0.2 | MIT OR Apache-2.0 | Numeric traits for generic mathematics |
| openssl | 0.10.57 | Apache-2.0 | OpenSSL bindings |
| p384 | 0.13.0 | Apache-2.0 OR MIT | Pure Rust implementation of the NIST P-384 (a.k.a. secp384r1) elliptic curve as defined in SP 800-186 with support for ECDH, ECDSA signing/verification, and general purpose curve arithmetic support. |
| rand | 0.8 | MIT OR Apache-2.0 | Random number generators and other randomness functionality. |
| rand_core | 0.6.4 | MIT OR Apache-2.0 | Core random number generator traits and tools for implementation. |
| rayon | 1.5 | MIT OR Apache-2.0 | Simple work-stealing parallelism for Rust |
| rcgen | 0.11.3 | MIT OR Apache-2.0 | Rust X.509 certificate generator |
| rug | ~1.23.0 | LGPL-3.0+ | Arbitrary-precision integers, rational, floating-point and complex numbers based on GMP, MPFR and MPC. |
| serde | 1.0 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| serde-wasm-bindgen | 0.4 | MIT | Native Serde adapter for wasm-bindgen |
| sha2 | 0.10.7 | MIT OR Apache-2.0 | Pure Rust implementation of the SHA-2 hash function family including SHA-224, SHA-256, SHA-384, and SHA-512. |
...
| clap | 4.4.6 | MIT OR Apache-2.0 | A simple to use, efficient, and full-featured Command Line Argument Parser |
| criterion | 0.5.1 | Apache-2.0 OR MIT | Statistics-driven micro-benchmarking library |
| csv | 1.3.0 | Unlicense/MIT | Fast CSV parsing with support for serde. |
| hex | 0.4 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| num-bigint | 0.4 | MIT/Apache-2.0 | Big integer implementation for Rust |
| ordered-float | 4.4.0 | MIT | Wrappers for total ordering on floats |
| rand | 0.8.5 | MIT OR Apache-2.0 | Random number generators and other randomness functionality. |
| rayon | 1.8.0 | MIT OR Apache-2.0 | Simple work-stealing parallelism for Rust |
| rusqlite | 0.32 | MIT | Ergonomic wrapper for SQLite |
| serde | 1.0.188 | MIT OR Apache-2.0 | A generic serialization/deserialization framework |
| serde_json | 1.0.107 | MIT OR Apache-2.0 | A JSON serialization file format |
| strum | 0.25.0 | MIT | Helpful macros for working with enums and strings |
| strum_macros | 0.25.2 | MIT | Helpful macros for working with enums and strings |
| tempfile | 3.15.0 | MIT OR Apache-2.0 | A library for managing temporary files and directories. |
| tokio | 1.32.0 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| uuid | 1.4.1 | Apache-2.0 OR MIT | A library to generate and parse UUIDs. |
...
|------------|---------|---------|-------------|
| anyhow | 1.0.75 | MIT OR Apache-2.0 | Flexible concrete Error type built on std::error::Error |
| async-trait | 0.1.74 | MIT OR Apache-2.0 | Type erasure for async trait methods |
| async_once | 0.2.6 | MIT OR Apache-2.0 | async once tool for lazy_static |
| aws-config | 1.6.0 | Apache-2.0 | AWS SDK config and credential provider implementations. |
| aws-sdk-s3 | 1.79.0 | Apache-2.0 | AWS SDK for Amazon Simple Storage Service |
| aws-sdk-secretsmanager | 1.66.0 | Apache-2.0 | AWS SDK for AWS Secrets Manager |
| aws-sdk-sesv2 | 1.70.0 | Apache-2.0 | AWS SDK for Amazon Simple Email Service |
| aws-sdk-sns | 1.63.0 | Apache-2.0 | AWS SDK for Amazon Simple Notification Service |
| aws-smithy-types | 1.3.0 | Apache-2.0 | Types for smithy-rs codegen. |
| base64 | 0.21.0 | MIT OR Apache-2.0 | encodes and decodes base64 as bytes or utf8 |
...
| config | 0.13.1 | MIT OR Apache-2.0 | Layered configuration system for Rust applications. |
| croner | 2.0 | MIT | Fully-featured, lightweight, and efficient Rust library designed for parsing and evaluating cron patterns |
| csv | 1.3.0 | Unlicense/MIT | Fast CSV parsing with support for serde. |
| deadpool | 0.10.0 | MIT OR Apache-2.0 | Dead simple async pool |
| deadpool-postgres | 0.11.0 | MIT OR Apache-2.0 | Dead simple async pool for tokio-postgres |
| dotenv | 0.15.0 | MIT | A `dotenv` implementation for Rust |
| ecies | 0.2 | MIT | Elliptic Curve Integrated Encryption Scheme for secp256k1 |
| encoding_rs | 0.8.34 | (Apache-2.0 OR MIT) AND BSD-3-Clause | A Gecko-oriented implementation of the Encoding Standard |
| encoding_rs_io | 0.1.7 | MIT OR Apache-2.0 | Streaming transcoding for encoding_rs |
| flate2 | 1.0.28 | MIT OR Apache-2.0 | DEFLATE compression and decompression exposed as Read/BufRead/Write streams. Supports miniz_oxide and multiple zlib implementations. Supports zlib, gzip, and raw deflate streams. |
| fs_extra | 1.3.0 | MIT | Expanding std::fs and std::io. Recursively copy folders with information about process and much more. |
| futures | 0.3.30 | MIT OR Apache-2.0 | An implementation of futures and streams featuring zero allocations, composability, and iterator-like interfaces. |
| graphql_client | 0.13.0 | Apache-2.0 OR MIT | Typed GraphQL requests and responses |
| handlebars | 6.1.0 | MIT | Handlebars templating implemented in Rust. |
| hex | 0.4 | MIT OR Apache-2.0 | Encoding and decoding data into/from hexadecimal representation. |
| keycloak | 24.0.301 | Unlicense OR MIT | Keycloak Admin REST API. |
| lapin | 2.5 | MIT | AMQP client library |
| lazy_static | 1.4.0 | MIT OR Apache-2.0 | A macro for declaring lazily evaluated statics in Rust. |
| lettre | 0.11 | MIT | Email client |
| num_cpus | 1.16.0 | MIT OR Apache-2.0 | Get the number of CPUs on a machine. |
| once_cell | 1.20.3 | MIT OR Apache-2.0 | Single assignment cells and lazy values. |
| openssl | 0.10.57 | Apache-2.0 | OpenSSL bindings |
| ordered-float | 4.4.0 | MIT | Wrappers for total ordering on floats |
| postgres-openssl | 0.5.0 | MIT OR Apache-2.0 | TLS support for tokio-postgres via openssl |
| quick-error | 2.0.1 | MIT/Apache-2.0 | A macro which makes error types pleasant to write. |
| rand | 0.8.5 | MIT OR Apache-2.0 | Random number generators and other randomness functionality. |
| rayon | 1.5 | MIT OR Apache-2.0 | Simple work-stealing parallelism for Rust |
| regex | 1.10.2 | MIT OR Apache-2.0 | An implementation of regular expressions for Rust. This implementation uses finite automata and guarantees linear time matching on all inputs. |
| reqwest | 0.12.5 | MIT OR Apache-2.0 | higher level HTTP client library |
...
| rocket | 0.5.0 | MIT OR Apache-2.0 | Web framework with a focus on usability, security, extensibility, and speed. |
| rusqlite | 0.32 | MIT | Ergonomic wrapper for SQLite |
| rust-s3 | 0.33.0 | MIT | Rust library for working with AWS S3 and compatible object storage APIs |
...
| serde_path_to_error | 0.1 | MIT OR Apache-2.0 | Path to the element that failed to deserialize |
| sha2 | 0.10 | MIT OR Apache-2.0 | Pure Rust implementation of the SHA-2 hash function family including SHA-224, SHA-256, SHA-384, and SHA-512. |
| structopt | 0.3.26 | Apache-2.0 OR MIT | Parse command line argument by defining a struct. |
| strum | 0.25 | MIT | Helpful macros for working with enums and strings |
| strum_macros | 0.25 | MIT | Helpful macros for working with enums and strings |
| tar | 0.4.40 | MIT OR Apache-2.0 | A Rust implementation of a TAR file reader and writer. This library does not currently handle compression, but it is abstract over all I/O readers and writers. Additionally, great lengths are taken to ensure that the entire contents are never required to be entirely resident in memory all at once. |
| tempfile | 3.15.0 | MIT OR Apache-2.0 | A library for managing temporary files and directories. |
| thiserror | 1.0.50 | MIT OR Apache-2.0 | derive(Error) |
| time | 0.3.31 | MIT OR Apache-2.0 | Date and time library. Fully interoperable with the standard library. Mostly compatible with #![no_std]. |
| tokio | 1.32.0 | MIT | An event-driven, non-blocking I/O platform for writing asynchronous I/O backed applications. |
...
| toml | 0.8.0 | MIT OR Apache-2.0 | A native Rust encoder and decoder of TOML-formatted files and streams. Provides implementations of the standard Serialize/Deserialize traits for TOML data to facilitate deserializing and serializing Rust structures. |
| tracing | 0.1 | MIT | Application-level tracing for Rust. |
| tracing-subscriber | 0.3 | MIT | Utilities for implementing and composing `tracing` subscribers. |
| unicode-normalization | 0.1.24 | MIT/Apache-2.0 | This crate provides functions for normalization of Unicode strings, including Canonical and Compatible Decomposition and Recomposition, as described in Unicode Standard Annex #15. |
| uuid | 1.5.0 | Apache-2.0 OR MIT | A library to generate and parse UUIDs. |
| walkdir | 2.3 | Unlicense/MIT | Recursively walk a directory. |
| xz2 | 0.1.7 | MIT/Apache-2.0 | Rust bindings to liblzma providing Read/Write streams as well as low-level in-memory encoding/decoding. |
| zip | 2.1.6 | MIT | Library to support the reading and writing of zip files. |
