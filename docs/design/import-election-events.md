...
```.env.development
# Keys are relative to AWS_S3_PUBLIC_BUCKET and must not start with `/`.
...
Windmill fetches these templates from the public S3-compatible object store
when it creates a realm. Development, remote-compose, and air-gapped MinIO
deployments recursively upload the tracked `.devcontainer/minio/public-assets/`
tree through `configure-minio`. Production operators must sync that tree before
