...
```.env.development
# Keys are relative to the private AWS_S3_BUCKET and must not start with `/`.
...
deployments upload the tracked `.devcontainer/keycloak/import/` files through
`configure-minio`. Production operators must upload both objects before
