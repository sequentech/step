...
pub(super) fn sign_channels<C: Ctx>(
    ...
    assert_eq!(datalog::hashes_count(&channels_hs.0), *num_trustees, "Unexpected number of channels");
        ...
        .enumerate()
    {
        ...
        if !ok {
            return Err(ProtocolError::VerificationError(format!("Failed to verify schnorr proof on channel")));
        }
        ...
        if i == *self_pos {
            ...
            if *sk.pk_element() != pk_element {
                return Err(ProtocolError::VerificationError(format!("Failed to decrypt self channel")));
            }
        }
    }
...
/// 
/// First this trustee's Channel is retrieved, and
/// the private key is decrypted. This key is then used 
/// to decrypts the shares sent to this trustee, which are
/// verified using the commitments. The share commitments
/// are then used to compute the public key as well as
/// the all trustee's verification keys (used to verify 
/// decryptions).
/// 
