...
pub trait Board: Sized {
    ...
    fn get_messages(&mut self, last_id: Option<i64>) -> impl std::future::Future<Output = Result<Vec<(Message, i64)>>> + Send;
    fn insert_messages(&mut self, messages: Vec<Message>) -> impl std::future::Future<Output = Result<()>> + Send;
}
