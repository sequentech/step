...
impl WasmHttpBoard {
    ...
    async fn fetch_messages_internal(
        ...
        board_name: &str,
        last_id: i64,
    ) -> Result<Vec<HttpB3Message>, JsValue> {
        let url = format!(
            "{}/boards/{}/messages?last_id={}",
            self.params.b4_url, board_name, last_id
        );
        ...
        for msg in get_response.messages {
            ...
            let id: i64 = msg.message.id.parse().map_err(|e| {
    ...
    async fn post_message_internal(
        &self,
        board_name: &str,
        message: Message,
    ) -> Result<(), JsValue> {
        // Extract metadata
        let sender_pk = message
            .sender
            .pk
            .to_der_b64_string()
        ...
        if init_resp.should_upload {
            ...
            let upload_url = init_resp
                .upload_url
                .ok_or_else(|| JsValue::from_str("Server indicated upload but no URL provided"))?;
...
impl Board for WasmHttpBoard {
    ...
    async fn get_messages(
        ...
        board_name: &str,
        last_id: i64,
    ) -> anyhow::Result<Vec<HttpB3Message>> {
