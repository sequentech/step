...
const DATABASE_USER: &str = "b3_test";
...
fn checked(output: std::io::Result<Output>) {
    let output = output.expect("install PostgreSQL and set PG_BIN to its bin directory");
    assert!(
...
impl Postgres {
    ...
    pub fn parameters(&self) -> b3::client::pgsql::PgsqlDbConnectionParams {
        b3::client::pgsql::PgsqlConnectionParams::new(
