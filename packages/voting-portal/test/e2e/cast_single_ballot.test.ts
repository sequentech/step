...
describe("login", function () {
	...
	it("should be able to cast ballot for all available elections", async (browser: NightwatchAPI) => {
		...
		const namesOfElections = electionList.map(async function (electionItem) {
			...
			const electionTitle = await browser.elementIdText((Object.values(electionItem)[0] as string))
		...
		const asyncFns = namesOfElections.map((nameOfElection) => {
			...
			return () => new Promise((res, rej) => {
				...
				nameOfElection.then(e=>{
					console.log({e})
					browser.useXpath().click(`//p[contains(normalize-space(),'${e}') and contains(@class,'election-title')]/../../div/button[normalize-space()='Click to Vote']`)
					...
					browser.elements('css selector', `.contest-title`, function (contestList) {
						...
						contestList.value.forEach(contestItem => {
							...
							browser.elementIdText((Object.values(contestItem)[0] as string), function (contestTitle) {
								...
								browser.elements('xpath', `//h5[normalize-space()='${contestTitle.value}']/..//div[contains(@class, 'candidate-item')]`, function (candidateList) {
									// console.log({res})
									candidateList.value.forEach(function (el) {
										console.log({ el })
									})
	
									browser.useXpath().click(`//h5[normalize-space()='${contestTitle.value}']/..//div[contains(@class, 'candidate-item')][${Math.floor(Math.random() * candidateList.value.length) + 1
										}]`)
		...
		// //loop through collected names of elections to cast ballot for them
		// console.log({namesOfElections})
		// namesOfElections.forEach((nameOfElection)=>{
		// 	console.log({nameOfElection})
		// 	// browser.debug()
		// 	browser.useXpath().click(`//p[contains(normalize-space(),'${nameOfElection}') and contains(@class,'election-title')]/../../div/button[normalize-space()='Click to Vote']`)
		// 		.click(`//button[normalize-space()="Start Voting"]`)
