...
public class MessageOTPCredentialModel extends CredentialModel {
    ...
    private static Optional<CredentialModel> 
        getOtpCredentialModel(@NotNull UserModel user)
    {
            ...
            .ifPresent(credential -> {
                try {
                    credential.setCredentialData(
                        JsonSerialization.writeValueAsString(credentialData)
                    );
                    credential.setSecretData(
                        JsonSerialization.writeValueAsString(
                            new OTPSecretData(secretValue)
                        )
                    );
                    MessageOTPCredentialModel credentialModel = 
                        MessageOTPCredentialModel
                            .createFromCredentialModel(credential);
                    user
                        .credentialManager()
                        .updateStoredCredential(credentialModel);
    ...
    public static MessageOTPCredentialModel createFromCredentialModel(
        ...
        try {
            MessageOTPCredentialData credentialData = JsonSerialization
                .readValue(
                    credentialModel.getCredentialData(),
                    MessageOTPCredentialData.class
                );
            MessageOTPSecretData secretData = JsonSerialization
                .readValue(
                    credentialModel.getSecretData(),
                    MessageOTPSecretData.class
                );
    ...
    public static class MessageOTPCredentialData {
        ...
        @JsonCreator
        public MessageOTPCredentialData(
            @JsonProperty("isSetup")boolean isSetup
        ) {
