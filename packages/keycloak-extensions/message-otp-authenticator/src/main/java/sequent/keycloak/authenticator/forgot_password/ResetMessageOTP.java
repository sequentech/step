    ...
    extends AbstractSetRequiredActionAuthenticator
{
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        if (
            ...
            Optional<String> credentialId = user
                .credentialManager()
                .getStoredCredentialsStream()
                .filter(credential -> {
                    boolean result = credential
                        .getType()
                        .equals(MessageOTPCredentialModel.TYPE);
                    log.infov(
            ...
            log.infov(
                "authenticate(): credentialId.isPresent()={0}",
                credentialId.isPresent()
            );
            if (credentialId.isPresent()) {
                log.infov(
                    "authenticate(): removing credentialId={0}",
                    credentialId
                );
                user
                    .credentialManager()
                    .removeStoredCredentialById(credentialId.get());
            }
        }
        context.success();
    }
