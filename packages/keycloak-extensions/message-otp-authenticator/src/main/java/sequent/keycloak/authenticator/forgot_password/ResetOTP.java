...
/**
 * This is similar to the stock Reset OTP, but with some changes:
 * - configuredFor() returns always true. This allows us to put it as required
 *   in a flow without cancelling the flow if the user doesn't have an OTP.
 * - It is not configurable, it just deletes all the otp credentials
 * - It doesn't add a ResetOTP RequiredAction. In our case that is performed
 *   by the MFA MethodSelector.
 */
@JBossLog
    ...
    implements CredentialValidator<OTPCredentialProvider>
{
    ...
    public void authenticate(AuthenticationFlowContext context)
    {
        ...
        List<CredentialModel> otpCredentialModelList = context
            .getUser()
            .credentialManager()
        ...
        otpCredentialModelList
            .forEach(otpCredentialModel -> {
    ...
    public OTPCredentialProvider getCredentialProvider(KeycloakSession session) {
        return (OTPCredentialProvider)session
