...
import sequent.keycloak.authenticator.smart_link.SmartLinkAuthenticatorFactory;
...
/**
 * Common utilities Smart Link authentication, used by the authenticator
 * and resource
 **/
@JBossLog
public class SmartLink {
    ...
    public static final String SMART_LINK_PROVIDER_ID =
        SmartLinkAuthenticatorFactory.PROVIDER_ID;
    ...
    public static SmartLinkActionToken createActionToken(
        ...
        String scopes = authSession
            .getClientNote(OIDCLoginProtocol.SCOPE_PARAM);
        String state = authSession.getClientNote(OIDCLoginProtocol.STATE_PARAM);
        String nonce = authSession.getClientNote(OIDCLoginProtocol.NONCE_PARAM);
        log.infof(
            "Attempting SmartLinkAuthenticator for %s, %s, %s",
            user.getEmail(),
            clientId,
            redirectUri
        );
        log.infof(
            "SmartLinkAuthenticator extra vars %s %s %s %b",
            scopes,
            state,
            nonce,
            rememberMe
    ...
    public static String linkFromActionToken(
        ...
        if (Config.getAdminRealm().equals(realm.getName()))
        {
            throw new IllegalStateException(
                String.format(
                    "Smart links not allowed for %s realm",
    ...
    private static UriBuilder actionTokenBuilder(
        URI baseUri,
        String tokenString,
        String clientId
    ) {
        log.debugf(
            "baseUri: %s, tokenString: %s, clientId: %s",
            baseUri,
            tokenString,
            clientId
        );
            ...
            new KeycloakSessionTask() {
                ...
                public void run(KeycloakSession session) {
                    try {
                        setupDefaultFlow(
                            session,
                            session.realms().getRealmByName(name)
                        );
                    } catch (Exception error) {
                        log.warn(
                            "Error setting up default smart link flow",
                            error
                        );
                    }
                }
    ...
    public static void setupDefaultFlow(
        KeycloakSession session, 
        RealmModel realm
    ) {
        AuthenticationFlowModel flow = realm.
            getFlowByAlias(SMART_LINK_AUTH_FLOW_ALIAS);
        if (flow != null) {
        ...
        log.infof(
            "creating built-in auth flow for %s",
            SMART_LINK_AUTH_FLOW_ALIAS
        );
        ...
        forms.setAlias(
            String.format("%s %s", SMART_LINK_AUTH_FLOW_ALIAS, "forms")
        );
    ...
    private static int getNextPriority(
        ...
        List<AuthenticationExecutionModel> executions = realm
            .getAuthenticationExecutionsStream(parentFlow.getId())
            .collect(Collectors.toList());
        return (executions.isEmpty())
            ? 0
            : executions.get(executions.size() - 1).getPriority() + 1;
    }
