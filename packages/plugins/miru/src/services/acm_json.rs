...
use crate::{
    bindings::plugins_manager::transactions_manager::vault::{read_secret, save_secret},
    services::{
        eml_types::ACMTrustee,
        signatures::{generate_ecies_key_pair, EciesKeyPair},
    },
};
...
#[instrument(err)]
pub fn get_acm_key_pair(tenant_id: &str, election_event_id: &str) -> Result<EciesKeyPair, String> {
    let secret_key = format!(
        "acm-key-pair-{}-{}",
        get_miru_device_id(),
        get_miru_serial_number()
    );
    if let Some(secret_str) =
