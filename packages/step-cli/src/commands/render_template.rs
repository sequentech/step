...
fn render_template_with_vars(
    ...
    let base_html_template_fild_path = "/workspaces/step/packages/velvet/src/resources/report_base_html.hbs";

    let base_template_content = fs::read_to_string(&base_html_template_fild_path)
        .map_err(|e| format!("Could not read template file {}: {}", base_html_template_fild_path, e))?;
