...
impl CreateApplications {
    ...
    pub async fn run_create_applications(
        ...
        let mut users_params: Vec<Vec<Box<dyn tokio_postgres::types::ToSql + Sync>>> = Vec::new();
        for user in users {
            ...
            let id_card_number =
                format!("C{}", NumberWithFormat(EN, "##########").fake::<String>());
            applicant_data.insert(
    ...
    async fn get_permission_label(
        &self,
        hasura_transaction: &tokio_postgres::Transaction<'_>,
        tenant_id: &str,
