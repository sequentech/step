...
#[derive(Args, Debug)]
#[command(about = "Create a new tenant", long_about = None)]
pub struct CreateTenant {
    /// Slug - unique short identifier for the new tenant
    #[arg(long)]
    slug: String,
}
...
impl CreateTenant {
    pub fn run(&self) {
        match create_tenant(&self.slug) {
            Ok(id) => {
                println!(
                    "{} {}",
                    "Success! Successfully created tenant. ID:".green(),
                    id.cyan()
                );
            }
            Err(err) => {
                eprintln!("Error! Failed to create tenant: {}", err)
            }
        }
    }
}
...
fn create_tenant(slug: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    if response.status().is_success() {
        let response_body: Response<insert_tenant::ResponseData> = response.json()?;
        if let Some(data) = response_body.data {
            let Some(output) = data.insert_tenant else {
                return Err(Box::from("failed creating tenant"));
            };
