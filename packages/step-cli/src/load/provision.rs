...
use crate::adapters::load_setup::{AdministratorProvisioning, LocalPreparation};
...
pub fn fixture(event: Value, settings: &Settings) -> Result<Value> {
    fixture_with(event, settings, &LocalPreparation)
}

fn fixture_with(
    mut event: Value,
    settings: &Settings,
    environment: &impl PreparationEnvironment,
) -> Result<Value> {
    ...
    let alias = format!("Synthetic load {}", environment.fixture_id());
...
fn import_census(
    input: &Input,
    directory: &Path,
    api: &impl Provisioning,
    environment: &impl PreparationEnvironment,
) -> Result<()> {
    environment.generate_census(input, directory)?;
...
pub(super) fn setup_with(
    ...
    if let Some(existing) = existing {
        ...
        input.validate()?;
        import_census(&input, &output.join("census"), api, environment)?;
    ...
    let fixture = fixture_with(files::read(&template)?, &settings, environment)?;
    let fixture_path = output.join("fixture.json");
    files::save(&fixture_path, &fixture)?;
    let event_id = api.import_event(
        fixture_path
            .to_str()
    ...
    import_census(&input, &output.join("census"), api, environment)?;
    let ceremony = api.start_ceremony(&event_id, settings.preparation.threshold.try_into()?)?;
    files::save(
        &output.join("setup-state.json"),
