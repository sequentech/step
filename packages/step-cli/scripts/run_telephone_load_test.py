...
(setup_telephone_load_test.py: a tenants.json index plus one
summary.json + voters CSV per tenant), generates a local phone_config.json
and one DTMF input script per voter from a captured call template, then fans
out N parallel `ivr-cli --bundle dev` processes per tenant — each an
independent simulated phone call against the dev container's real
Keycloak/Hasura. Tenants are processed one after another; every tenant Stage
1 provisioned gets called. See
...
def place_calls_for_tenant(
    *,
    tenant_id: str,
    tenant_summary_json: Path,
    tenant_out_dir: Path,
    template_text: str,
    concurrency: int,
    max_calls: int | None,
    ...
    call_timeout: int,
    system_number: str,
    success_regex: re.Pattern,
    ivr_cli_bin: str,
    valkey_url: str,
    ivr_service_client_id: str,
    ivr_voting_client_id: str,
    ivr_service_client_secret: str,
    ivr_voting_client_secret: str,
    keycloak_url_override: str | None,
    hasura_url_override: str | None,
) -> dict:
    ...
    keycloak_url = keycloak_url_override or summary.get("keycloak_url")
    ...
    hasura_url = (hasura_url_override or summary.get("hasura_url") or "").removesuffix("/v1/graphql")
    ...
    if voting_channel and voting_channel != "TELEPHONE":
        common.die(
            f"{tenant_summary_json} was provisioned for the {voting_channel} channel — re-run "
    ...
    (tenant_out_dir / "inputs").mkdir(parents=True, exist_ok=True)
    (tenant_out_dir / "logs").mkdir(parents=True, exist_ok=True)
    common.log(f"Writing call inputs/logs to {tenant_out_dir}")
    ...
    phone_config_path = tenant_out_dir / "phone_config.json"
    ...
    exit_codes_path = tenant_out_dir / "exit_codes.csv"
    ...
    results_path = tenant_out_dir / "results.csv"
    cast = 0
    failed = 0
    with exit_codes_path.open() as f, results_path.open("w") as results_f:
        ...
        for line in f:
            ...
            log_text = (tenant_out_dir / "logs" / f"call-{voter_id}.log").read_text(errors="replace") if (tenant_out_dir / "logs" / f"call-{voter_id}.log").is_file() else ""
    ...
    tenant_summary_out = {
        ...
        "logs_dir": str(tenant_out_dir / "logs"),
    }
    common.write_json(tenant_out_dir / "summary.json", tenant_summary_out)
    common.log(f"Tenant {tenant_id} done in {elapsed}s: {cast}/{count} calls cast a ballot ({failed} did not)")
    return tenant_summary_out
...
def main() -> None:
    ...
    # Fallback secrets, used for any tenant not listed in tenant_ivr_secrets
    # below. IVR client secrets are election-event-realm-scoped, so with
    # more than one tenant these almost certainly differ per tenant — set
    # tenant_ivr_secrets for anything beyond a single-tenant run.
    default_ivr_service_secret = common.opt_str(cfg, "keycloak_ivr_service_client_secret", env="KEYCLOAK_IVR_SERVICE_CLIENT_SECRET")
    default_ivr_voting_secret = common.opt_str(cfg, "keycloak_ivr_voting_client_secret", env="KEYCLOAK_IVR_VOTING_CLIENT_SECRET")
    tenant_ivr_secrets = cfg.get("tenant_ivr_secrets") or {}
    ...
    for i, tenant in enumerate(tenants):
        ...
        common.log(f"=== Tenant {i + 1}/{len(tenants)}: {tenant_id} ===")
        overrides = tenant_ivr_secrets.get(tenant_id) or {}
        ivr_service_client_secret = overrides.get("keycloak_ivr_service_client_secret") or default_ivr_service_secret
        ivr_voting_client_secret = overrides.get("keycloak_ivr_voting_client_secret") or default_ivr_voting_secret
        if not ivr_service_client_secret:
            common.die(f"no IVR service client secret for tenant {tenant_id} (set telephone_run.tenant_ivr_secrets['{tenant_id}'] or the flat keycloak_ivr_service_client_secret default)")
        if not ivr_voting_client_secret:
            common.die(f"no IVR voting client secret for tenant {tenant_id} (set telephone_run.tenant_ivr_secrets['{tenant_id}'] or the flat keycloak_ivr_voting_client_secret default)")

        # Distinct system number per tenant so a single voter_id can't
        # collide across tenants' independent phone_config.json entries.
        tenant_system_number = system_number if i == 0 else f"+1{111111111111 + i}"

        tenant_summary = place_calls_for_tenant(
            tenant_id=tenant_id,
            tenant_summary_json=run_dir / tenant["dir"] / "summary.json",
            tenant_out_dir=out_dir / f"tenant-{tenant_id}",
            template_text=template_text,
            concurrency=concurrency,
            max_calls=max_calls,
            voter_offset=voter_offset,
            call_timeout=call_timeout,
            system_number=tenant_system_number,
            success_regex=success_regex,
            ivr_cli_bin=ivr_cli_bin,
            valkey_url=valkey_url,
            ivr_service_client_id=ivr_service_client_id,
            ivr_voting_client_id=ivr_voting_client_id,
            ivr_service_client_secret=ivr_service_client_secret,
            ivr_voting_client_secret=ivr_voting_client_secret,
            keycloak_url_override=keycloak_url_override,
            hasura_url_override=hasura_url_override,
        )
