...
// Generated human view (conventions 1+2: legend included, never hand-write).
const key = (xs) =>
    xs === null
        ? "unreachable"
        : xs.length === 0
          ? "—"
          : xs.map((s) => "`" + s.replace(/^errors\.\w+\./, "").replace(/\(.*\)$/, "") + "`").join("<br>")
const dlg = (d) =>
    d.kind === "none"
        ? "none"
        : d.kind === "dismissible"
          ? `dismissible (${(d.buttons ?? []).join("/")})`
          : `**blocking** (${(d.buttons ?? []).join("/")})`
const md = [
    "<!--",
    " SPDX-FileCopyrightText: 2026 Sequent Tech Inc <legal@sequentech.io>",
    "",
    ...
    ...rows.map(
        (r) =>
            `| ${r.blank_vote_policy} | ${key(r.untouched)} | ${key(r.voting_touched)} | ` +
            `${dlg(r.dialog)} | ${key(r.review)} |`
    ),
    "",
].join("\n")
writeFileSync(path.join(here, "blank-rule.filter.md"), md)
console.log("wrote blank-rule.filter.recorded.json and blank-rule.filter.md")
