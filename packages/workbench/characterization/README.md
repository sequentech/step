...
| **acclaimed contests** (upstream #3106, caught up 2026-08-30) — a contest with `is_acclaimed: Some(true)` is skipped by both gates (`votable_contests`), excluded from encoding, and gets a synthetic tally | out of scope: no bundled fixture sets the flag (`None` everywhere → every contest is votable → all recorded evidence is unaffected); the skip lives at the gates' composition layer, outside the per-contest domain the spec certifies | an acclaimed fixture, or the composition model (`BallotValidator`) growing a votability notion |
| **missing decoded record** (upstream #3089, caught up 2026-08-30) — a contest with no entry in the gates' decoded map hard-blocks (fail closed); the dialog does not fire | out of scope: every instrument supplies a record per contest, and the spec's ballot composition takes only existing (contest, record) pairs — the branch lives at the call site | the composition model growing absent-record handling, or evidence that drives the gates with a partial map |
...
**233/233 matching the spec**, every prediction drawn from `f_fixed`,
which the completed injection makes production's own implementation
(the same expectation as `headless-sweep.mjs`) (see the e2e-cost note in
[`../docs/VALIDATION_LOGIC_DISTILLATION.md`](../docs/VALIDATION_LOGIC_DISTILLATION.md)
§5.3).
...
|---|---|
| `headless-sweep.mjs` | `headless-sweep.md` + `.recorded.json` — production (real WASM checker/gates/tally) vs the Rust spec on **every** cell of the representable headless domain: all six policies × sane bounds × plurality states × the reachable preferential states; **345,600 cells, ~1 min**. The injection is COMPLETE — the gates (`voting_screen.rs`), decode (`raw_ballot.rs`) and the booth's message filter (`InvalidErrorsList.tsx`) all run the rationalized implementation, and the tally classifies from the decoded record — so production is compared against `f_fixed` on every effect. This is the exhaustive fidelity check, and it emits the quotient inventory the browser stage consumes |
...
|---|---|---|
| `dom-validate.mjs` | `dom-validate.md` + `.recorded.json` | the **complete** tables — the Rust spec's predictions vs the real DOM across every cell of all seven rules: inline visibility at **both** observation points (touched voting screen and review), reachability, and the untouched view asserted empty per cell; every prediction from `f_fixed` — production's own implementation, the injection being complete; 233/233, ~8 min |
| `quotient-validate.mjs` | `quotient-validate.md` + `.recorded.json` | the browser-side *independence* claims discharged by sufficiency — one booth-formable member per quotient class from `headless-sweep.md`, inline compared at both observation points, re-observed up to 3× on mismatch (the 2026-08-19 run needed it on 8 classes, which would otherwise have been recorded as disagreements); member selection matches the class key on `f_fixed`'s emissions (production's — the injection is complete), and whether review renders is decided by the chosen member's production gate — member-determined, not class-determined (the deliberate-blank exemption reads past the emissions); the license (the filter's props boundary) is source-verified against the rewritten filter and stated with its re-entry condition; classes with no formable member labelled. ~1 h |
| `browser-witnesses.mjs` | `browser-witnesses.md` + `.recorded.json` | every browser-pending dependence witness from `effect-dependencies.md` (inline-view and reachability components) driven through the real booth on a generic per-cell recipe, both cells compared against the spec; unobservable witnesses labelled. Every spec value is `f_fixed`'s — production's own implementation, the injection being complete |
...
| `overvote-e2e-pipeline.mjs` | `overvote-e2e-pipeline.recorded.json` | the S1 over-vote cell: booth → cast → decrypt → decode → tally in one continuous run. With the display fix injected it confirms the INFORMED, uninterrupted discount — `selectedMax` renders inline, no dialog, ballot still excluded |
| `minvote-e2e-pipeline.mjs` | `minvote-e2e-pipeline.recorded.json` | the min-vote family, same full pipeline, per-cell expectations: the three below-min cells confirm the informed, uninterrupted discount (the injected S1 display fix), and the S2 cell confirms the injected S2/S3 fix — the deliberate blank is counted (`blank_votes.explicit`), not discarded |
