...
// Characterization of the DUPLICATED-RANK rule (preferential only):
...
// push a `duplicatedPosition` error on a duplicate-rank ballot regardless of
// policy; the policy decides only which gate reacts.
const CELLS = RULE_SPECS["duprank-rule"]
...
const fmt = (r) =>
    `| ${r.duplicated_rank_policy} | ${r.invalid_vote_policy} | ${r.state} | ` +
