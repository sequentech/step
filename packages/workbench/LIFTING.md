...
| A7 | `resolve.alias` `^sequent-core$` → `<repo>/packages/sequent-core/pkg`, registered by default; `predev`/`prebuild` build that directory | The lifted booth imports `from "sequent-core"`. There are two possible sources and they are **not** interchangeable: (a) `packages/sequent-core/pkg/` — the wasm-pack output of the in-tree crate, gitignored, rebuilt by `scripts/prepare-sequent-core.mjs`; (b) `packages/node_modules/sequent-core/`, unpacked by yarn from the committed `rust/sequent-core-0.1.0.tgz` that voting-portal / ui-core / admin-portal / ballot-verifier each ship. The tarball is a *snapshot packed from that same `pkg/`* at whatever commit someone last ran `.devcontainer/scripts/build-sequent-core.sh`. **The workbench defaults to (a).** The tally half (velvet-core → velvet-wasm) always compiles the crate from source via a Cargo path dep, so (a) is the only setting where both halves of the encrypt → decrypt → tally loop run the same code. Defaulting to (b) caused two recurring failures: local Rust edits silently not appearing, and the booth disagreeing with the tally with no error — just wrong numbers. Selection is by explicit flag, never by whether `pkg/` happens to exist: `WORKBENCH_SEQUENT_CORE=tgz` skips the build and leaves the alias unregistered. If `pkg/` is missing in the default mode the config **throws with instructions** rather than falling back. `pkg/` is built with `--features=wasm,default_features`, not the `wasmtest` the devcontainer script uses for the tarball, because `strand/wasmtest` does not compile on this branch (obsolete openssl/FIPS backends removed); no sequent-core item is gated on `wasmtest`, so the exported surface matches. Note that Rust changes to non-`#[wasm_bindgen]` parts of sequent-core also reach the tally half via velvet-wasm regardless of this alias. | Config throws `sequent-core: packages/sequent-core/pkg is missing` — you ran vite directly instead of `yarn dev`; run `yarn build:sequent-core` or set `WORKBENCH_SEQUENT_CORE=tgz`. The active source is shown on the Diagnostics page (*Booth sequent-core*). If voting-portal renames the import, update the regex. |
...
2. **Categorize the first error** using the canary table below:
   ...
   | Config throws `sequent-core: packages/sequent-core/pkg is missing` | Ran vite directly, so `predev` never built it | A7 (`yarn build:sequent-core`, or `WORKBENCH_SEQUENT_CORE=tgz`) |
   | Booth behaves like an older sequent-core, or disagrees with the tally | Running with `WORKBENCH_SEQUENT_CORE=tgz` | A7 (check *Booth sequent-core* on Diagnostics) |
