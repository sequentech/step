...
/// multi-ballot holds a single ciphertext covering every contest.
...
/// The contests named by a ballot must be exactly the contests of its ballot
/// style, each named once.
fn check_contest_ids_match_style(
    contest_ids: &[&str],
    config: &BallotStyle,
) -> Result<(), BallotError> {
    let style_contest_ids: HashSet<&str> = config
        .contests
        .iter()
    ...
    if style_contest_ids.len() != config.contests.len() {
    ...
    if named_contest_ids != style_contest_ids {
...
mod tests {
    ...
    mod ciphertext_reconstruction {
        ...
        /// A ballot style that lists the same contest twice collapses to a
        /// single id in the expected set, so a ballot naming it once would
        /// otherwise cover fewer contests than the style encodes.
        #[test]
