...
pub fn create_tree_js(
    ...
    // parse input
    let areas: Vec<TreeNodeArea> =
        serde_wasm_bindgen::from_value(areas_json)
            .map_err(|err| format!("Error reading javascript areas: {err}"))?;
    ...
    contests_data_tree
        ...
        .map_err(|err| format!("Error serializing contests_data_tree: {err}"))
        .into_json()
}
...
pub fn get_contest_matches_js(
    ...
    area_contests
        .serialize(&serializer)
        .map_err(|err| format!("Error serializing area_contests: {err}"))
        .into_json()
}
