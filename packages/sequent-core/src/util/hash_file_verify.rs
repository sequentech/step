...
pub fn verify_file_sha256(
    ...
    let mut voters_file_buffer: Vec<u8> = vec![];
    f.read_to_end(&mut voters_file_buffer).map_err(|err| {
        HashFileVerifyError::IoError(
            "Error reading voters file from the temp file.".to_string(),
            err,
        )
    })?;

    let calculated_hash_result = hash_sha256(voters_file_buffer.as_slice());
