...
fn create_contest(
    ...
    let candidates: Vec<ballot::Candidate> = sorted_candidates
        ...
        .map(|(_i, candidate)| {
            ...
            Ok(ballot::Candidate {
                ...
                tenant_id: Some(candidate.tenant_id.clone()),
                election_event_id: Some(candidate.election_event_id.clone()),
                election_id: Some(contest.election_id.clone()),
                contest_id: Some(contest.id.clone()),
    ...
    Ok(ballot::Contest {
        id: contest.id.clone(),
        tenant_id: Some(contest.tenant_id),
        election_event_id: Some(contest.election_event_id),
        election_id: Some(contest.election_id.clone()),
        name: contest.name,
        name_i18n,
        ...
        alias_i18n,
        max_votes: Some(contest.max_votes.unwrap_or(0)),
        min_votes: Some(contest.min_votes.unwrap_or(0)),
        winning_candidates_num: Some(
            contest.winning_candidates_num.unwrap_or(1),
        ),
        voting_type: contest.voting_type,
        counting_algorithm: contest.counting_algorithm,
        is_encrypted: Some(contest.is_encrypted.unwrap_or(false)),
        candidates,
