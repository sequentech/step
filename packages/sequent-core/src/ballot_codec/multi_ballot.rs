...
impl BallotChoices {
    ...
    pub fn decode_mixed_radix(
        ...
        while accumulator > Zero::zero() {
            let base: BigUint = bases[index]
                .to_biguint()
                .ok_or_else(|| format!("Error converting to biguint: bases[index={index:?}]={val}", val=bases[index]))?;

            let remainder = &accumulator % &base;
            values.push(
                remainder
                .to_u64()
                .ok_or_else(|| format!("Error converting to u64 remainder={remainder}"))?
            );
