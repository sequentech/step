...
impl PgsqlB3Client {
    ...
    pub async fn insert_ballots<C: Ctx>(
        ...
        insert_ballots(self.client.borrow_mut(), board_name, ballots).await
    }
}
...
#[instrument(err, skip(client))]
async fn insert_ballots(client: &mut Client, board_name: &str, ballots: Message) -> Result<()> {
...
async fn insert(client: &mut Client, board_name: &str, messages: &[B3MessageRow]) -> Result<()> {
    ...
    let lock = "select pg_advisory_xact_lock(hashtext($1))".to_string();
...
pub(crate) mod tests {
    ...
    const PG_DATABASE: &str = "protocoldb";
    const PG_HOST: &str = "localhost";
    const PG_USER: &str = "postgres";
    const PG_PASSW: &str = "postgrespw";
