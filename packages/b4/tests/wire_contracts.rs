...
use b4::api_types::ContentType;
use b4::messages::newtypes::{ChannelsHashes, DecryptionFactorsHashes, SharesHashes};
...
#[test]
fn inline_bytes_and_object_keys_have_distinct_literal_json_shapes() {
    let bytes = vec![0, 255, 16];
    let inline = ContentType::Inline {
        data: bytes.clone(),
    };
    assert_eq!(
        serde_json::to_value(&inline).unwrap(),
        serde_json::json!({"message":"AP8Q"})
    );
    let decoded: ContentType = serde_json::from_str(r#"{"message":"AP8Q"}"#).unwrap();
    assert!(matches!(decoded, ContentType::Inline { data } if data == bytes));
    let object = ContentType::S3 {
        key: "board/messages/item".into(),
    };
    assert_eq!(
        serde_json::to_value(&object).unwrap(),
        serde_json::json!({"key":"board/messages/item"})
    );
    assert!(
        matches!(serde_json::from_str::<ContentType>(r#"{"key":"board/messages/item"}"#).unwrap(), ContentType::S3 { key } if key == "board/messages/item")
    );
}

#[test]
fn malformed_base64_and_missing_or_wrongly_typed_content_are_rejected() {
    // Empty inline content is valid; malformed encodings are not empty content.
    assert!(
        matches!(serde_json::from_str::<ContentType>(r#"{"message":""}"#).unwrap(), ContentType::Inline { data } if data.is_empty())
    );
    for input in [
        r#"{"message":"!"}"#,
        r#"{"message":"AP8"}"#,
        ...
        assert!(
            serde_json::from_str::<ContentType>(input).is_err(),
            "{input}"
        );
    }
}
