...
use handlebars::{
    BlockParamHolder, Context, Handlebars, Helper, HelperDef, HelperResult, JsonValue, Output,
...
thread_local! {
    static DIAGNOSTICS: std::cell::RefCell<Vec<serde_json::Value>> = const { std::cell::RefCell::new(Vec::new()) };
}
pub fn take_diagnostics() -> Vec<serde_json::Value> {
    DIAGNOSTICS.with(|items| std::mem::take(&mut *items.borrow_mut()))
}
fn record_fallback(name: &str) {
    DIAGNOSTICS.with(|items| {
        let mut items = items.borrow_mut();
        if items.len() < 100 {
            items.push(serde_json::json!({"severity":"warning","code":"helper-fallback","message":format!("Helper '{name}' could not format its input; the production fallback was used")}));
        }
    });
}
pub fn get_registry<'reg>() -> Handlebars<'reg> {
    ...
    reg.register_helper(
        "datetime",
        helper_wrapper_or(Box::new(HandlebarsChronoDateTime), String::from("-")),
    );
    reg.register_helper("inc", helper_wrapper_or(Box::new(inc), String::from("-")));
    reg.register_helper("inc2", helper_wrapper_or(Box::new(inc2), String::from("-")));
    ...
    reg.register_helper("sum", helper_wrapper_or(Box::new(sum), String::from("-")));
    reg.register_helper(
        "modulo",
    ...
    crate::localization::register(&mut reg);
    reg
}
...
pub fn sanitize_html(
    ...
    let tags: HashSet<&str> = ["strong", "em", "b", "i", "br"].iter().cloned().collect();
...
fn parse_u64_value(value: &JsonValue) -> Result<u64, RenderError> {
    match value {
        JsonValue::Number(n) => n
            .as_u64()
            .ok_or_else(|| RenderError::new(format!("Expected u64 but got invalid number: {n}"))),
        JsonValue::String(s) => s
            .parse::<u64>()
            .map_err(|_| RenderError::new(format!("Failed to parse '{}' as u64", s))),
...
fn parse_f64_value(value: &JsonValue) -> Result<f64, RenderError> {
    match value {
        JsonValue::Number(n) => n
            .as_f64()
            .ok_or_else(|| RenderError::new(format!("Expected f64 but got invalid number: {n}"))),
        JsonValue::String(s) => s
            .parse::<f64>()
            .map_err(|_| RenderError::new(format!("Failed to parse '{}' as f64", s))),
        _ => Err(RenderError::new(
...
impl HelperDef for parse_i64 {
    fn call_inner<'reg: 'rc, 'rc>(
        ...
        let num_i64: i64 = num_str
            .parse::<i64>()
            .map_err(|_| RenderErrorReason::InvalidParamType("couldn't parse as i64"))?;
...
pub fn timestamp_to_rfc2822(timestamp: i64) -> Result<String> {
    let dt =
        DateTime::<Utc>::from_timestamp(timestamp, 0).with_context(|| "Error parsing timestamp")?;
...
pub fn to_json(
    ...
    if let Some(param) = h.param(0) {
        ...
        let json = to_string(param.value()).unwrap_or_else(|_| "null".to_string());
...
mod tests {
    ...
    fn url_encode_keeps_dynamic_data_in_one_query_value() {
        ...
        variables.insert("value".to_string(), json!("a&admin=true 50% \"Málaga\""));
    ...
    fn url_encode_rejects_non_string_values() {
        ...
        assert!(render_template_text("{{url_encode value}}", variables).is_err());
    }
}
