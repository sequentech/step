...
pub async fn set_custom_url(
    ...
    if let Ok(_cloudflare_env) = std::env::var("CLOUDFLARE_ENV") {
        let current_page_rule = get_page_rule("https://google.com").await?;
...
fn create_payload(origin: &str, redirect_to: &str) -> CreatePageRuleRequest {
    let url = format!("https://{}.vaiphon.com/login", origin);
...
fn create_dns_payload(origin: &str, redirect_to: &str) -> CreateDNSRecordRequest {
