...
use windmill::services::database::{get_hasura_pool, get_keycloak_pool, PgConfig};
...
pub async fn get_election_event_stats(
    ...
    let mut hasura_db_client: DbClient = get_hasura_pool()
        ...
        .get()
        .await
        .map_err(|err| (Status::InternalServerError, format!("Error loading hasura db client: {err}")))?;
    let mut hasura_transaction = hasura_db_client
        .transaction()
        .await
        .map_err(|err| (Status::InternalServerError, format!("Error creating a transaction: {err}")))?;
    ...
    let total_distinct_voters: i64 = 
        ...
        .await
        .map_err(|err| (Status::InternalServerError, format!("Error retrieving total_distinct_voters: {err}")))?;
