...
/// Datafix annotations; returning them lets each handler pass the values on to
/// the service layer instead of resolving the same event a second time.
struct InboundVoterLock {
    lock: PgLock,
