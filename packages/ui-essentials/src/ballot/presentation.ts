...
/**
 * What a contest's presentation settings mean, read off the platform's own types.
 *
 * Moved here from `voting-portal/src/services/ElectionConfigService.ts`. Every
 * function is a pure property read — "does this contest allow write-ins", "is this
 * candidate the blank option", "how many of these are checkable" — with no store,
 * no network and no WebAssembly, so it belongs beside the components that ask the
 * questions rather than in an application's services directory.
 *
 * It had to move for the ballot to move: `Question`, `Answer` and `AnswersList`
 * call nine of these between them, and two of them (`checkIsCategoryList`,
 * `checkIsExplicitBlankVote`) were already re-exports of `ui-core`. The portal
 * re-exports this module from its old path so its screens keep working.
 */
import {
    CandidatesOrder,
    ICandidate,
