...
/// Derives the v2 serialization traits (`SERIALIZATION.md` §9) for structs.
///
...
/// There is no tuple conversion and no arity limit — the generated code is
/// exactly what would be written by hand:
///
/// ```ignore
...
/// ```
///
/// Also emits `std::hash::Hash` via the serialized bytes, like the v1 derive.
///
/// **Spike status**: coexists with the v1 derive during the rewrite spike; at
/// flip time this becomes the only derive.
#[proc_macro_derive(VSer2)]
pub fn vser2_derive(input: TokenStream) -> TokenStream {
    let ast = parse_macro_input!(input as DeriveInput);

    impl_v2(&ast)
}

fn impl_v2(ast: &syn::DeriveInput) -> TokenStream {
    let name = &ast.ident;
    ...
    match &ast.data {
        syn::Data::Struct(s) => match &s.fields {
            syn::Fields::Named(fields) => {
                ...
                write_stmts = quote! { #( VSerializable::write(&self.#names, out); )* };
                read_ctor = quote! { Self { #( #names: <#tys as VDeserializable>::read(input)?, )* } };
            }
            syn::Fields::Unnamed(fields) => {
                let indices = (0..fields.unnamed.len()).map(syn::Index::from);
                let tys: Vec<_> = fields.unnamed.iter().map(|f| &f.ty).collect();
                write_stmts = quote! { #( VSerializable::write(&self.#indices, out); )* };
                read_ctor = quote! { Self( #( <#tys as VDeserializable>::read(input)?, )* ) };
            }
            syn::Fields::Unit => {
                write_stmts = quote! {};
        ...
        _ => {
            return quote! { compile_error!("VSer2 can only be derived for structs; enums implement the traits by hand (mini-spec rule 7)."); }
                .into();
        }
    }

    let generated = quote! {
        impl #impl_generics ::cryptography::utils::serialization::v2::VSerializable for #name #ty_generics #where_clause {
            fn write(&self, out: &mut Vec<u8>) {
                use ::cryptography::utils::serialization::v2::VSerializable;
                #write_stmts
            }
        }

        impl #impl_generics ::cryptography::utils::serialization::v2::VDeserializable for #name #ty_generics #where_clause {
            fn read(input: &mut &[u8]) -> Result<Self, ::cryptography::utils::error::Error> {
                use ::cryptography::utils::serialization::v2::{VSerializable, VDeserializable};
                Ok(#read_ctor)
            }
        }
