...
use crate::native::board::{HttpB5, HttpB5BoardParams};
...
pub struct SessionMaster<C: Context> {
    ...
    board_params: HttpB5BoardParams,
...
impl<C: Context> SessionMaster<C> {
    ...
    pub async fn new(b3_url: &str, session_factory: SessionFactory<C>, size: usize) -> Result<Self> {
        let board_params = HttpB5BoardParams::new(b3_url);
    ...
    pub fn run(mut self) -> JoinHandle<()> {
        let handler = tokio::spawn(async move {
            ...
            loop {
                ...
                let board: HttpB5 = <HttpB5BoardParams as BoardFactoryMulti<C, HttpB5>>::get_board(&self.board_params);
                let responses = <HttpB5 as BoardMulti<C>>::get_messages_multi(&board, &requests).await;
