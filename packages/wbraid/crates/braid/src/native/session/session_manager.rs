...
use crate::native::board::{HttpB4, HttpB4BoardParams};
...
pub struct SessionMaster<C: Context> {
    ...
    board_params: HttpB4BoardParams,
...
impl<C: Context> SessionMaster<C> {
    ...
    pub async fn new(b3_url: &str, session_factory: SessionFactory<C>, size: usize) -> Result<Self> {
        let board_params = HttpB4BoardParams::new(b3_url);
    ...
    pub fn run(mut self) -> JoinHandle<()> {
        let handler = tokio::spawn(async move {
            ...
            loop {
                ...
                let board: HttpB4 = <HttpB4BoardParams as BoardFactoryMulti<C, HttpB4>>::get_board(&self.board_params);
                let responses = <HttpB4 as BoardMulti<C>>::get_messages_multi(&board, &requests).await;
                ...
                let km_map: HashMap<String, Vec<HttpB4Message>> = HashMap::from_iter(tuples);
                ...
                if post_messages.len() > 0 {
                    ...
                    let http_post_messages: Vec<(String, Vec<b4::HttpB4Message>)> = post_messages
                        .into_iter()
