...
impl<C: Ctx> LocalBoardStorage<C> for LocalBoard<C> {
    fn add(&mut self, message: VerifiedMessage, store_id: i64) -> Result<(), ProtocolError> {
        Self::add(self, message, store_id)
    }
    ...
    fn get_statement_entries(&self) -> Vec<BoardEntry> {
        // self.statements.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
        self.get_statement_entries()
    }
...
impl<C: Ctx> LocalBoard<C> {
    ...
    fn get_dkg_artifact(
        ...
        hash: Hash,
        signer_position: TrusteePosition,
    ) -> Result<ArtifactRef<'_, Vec<u8>>, ProtocolError> {
    ...
    /// specified by last_local_board_id. This parameter refers to the last
    /// locally-controlled store ID that the board _has_ seen.
    ///
    ...
    pub(crate) fn store_and_return_messages(
        ...
        // SECURITY CRITICAL: The ORDER BY id ASC clause ensures that the trustee
        // processes messages in the order established by our local AUTOINCREMENT ID,
        // preventing the bulletin board from manipulating history by reordering or
        // prepending messages. See self::get_store.
    ...
    ///
    /// OPTIMIZATION ONLY: Used to request messages from the bulletin board
    /// with external_id > this value. Has NO security implications.
    ///
    /// If the store is empty -1 will be returned as a lower bound
    /// on all possible message ids. Note that the last external_id
    /// seen by the store is _not_ the same as the last local_id
    /// (AUTOINCREMENT) seen by the board.
    ...
    ///
    /// SECURITY MODEL:
    /// - The autogenerated 'id' column (AUTOINCREMENT) is locally-controlled and determines
    ///   message processing order. Once a retrieved message is stored and assigned a local id,
    ///   it is impossible for later messages to have an earlier id. The external bulletin board
    ///   can therefore NOT alter history by prepending or reordering messages.
    /// - The 'external_id' column stores the bulletin board's ID for optimization only
    ///   (to avoid re-fetching messages). It has NO security implications.
    ///
