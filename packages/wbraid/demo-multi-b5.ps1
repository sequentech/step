# Multi-Board Automated Demo Script for B5/braid_b5
...
function Cleanup {
    ...
    Get-Process | Where-Object { $_.ProcessName -match "main_concurrent|b5|monitor" } | Stop-Process -Force -ErrorAction SilentlyContinue
    
    # Stop background jobs
    Get-Job | Where-Object { $_.Name -match "b5" } | Stop-Job -ErrorAction SilentlyContinue
    Get-Job | Where-Object { $_.Name -match "b5" } | Remove-Job -ErrorAction SilentlyContinue
    
    # Stop PowerShell windows
    Get-Process powershell -ErrorAction SilentlyContinue | Where-Object { 
        $_.MainWindowTitle -match 'Trustee_|b5 Bulletin|monitor' 
    } | ForEach-Object {
...
if (-not $SkipCleanup) {
    # Kill all related processes including b5
    Get-Process powershell -ErrorAction SilentlyContinue | Where-Object { 
    ...
    Get-Process -ErrorAction SilentlyContinue | Where-Object { 
        $_.ProcessName -match '^(cargo|b5|main|monitor)$' 
    } | ForEach-Object {
...
# Set all required environment variables for b5 + LocalStack
$env:RUST_LOG = "b5=info,wbraid_service=info"
...
$monitorProcess = Start-Process powershell -ArgumentList @(
    "-NoExit",
    "-Command",
    "cd '$workingDir'; cargo run --package b5 --bin monitor --release --features monitor -- --db sqlite://b4.db"
) -PassThru
...
Write-Host ""
Write-Host "The B5 protocol is now running across $NumBoards boards concurrently." -ForegroundColor Cyan
...
while ($elapsed -lt $maxWaitSeconds -and -not $allComplete) {
    ...
    foreach ($boardName in $boardNames) {
        try {
            $output = cargo run --package braid_b5 --bin demo_tool --release -- list-messages `
...
Write-Host "  [OK] B5 protocol with cryptography library" -ForegroundColor Green
Write-Host "  [OK] braid_b5 trustees operational" -ForegroundColor Green
...
Write-Host "  - b5 server (PID: $($b5Process.Id))" -ForegroundColor Green
...
Write-Success "B5 Demo complete!"
