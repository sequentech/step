...
//! The global target: critical-path latency of one tally — **fork-point build**.
//!
//! A frozen copy of `crates/vsc/examples/tally.rs` for measuring commits that
//! predate `strip_all` (the fork point `657cb05c20` of the performance work):
//! identical composition and timing, with the two strips written as the
//! per-item `strip` in parallel, which is what the milestone's `targets.rs`
//! also measured at the fork point. `bench-ec2/remote-bench.sh` copies this
//! directory into such a baseline (`BASE_EXAMPLES_DIR`). Never edit it to track
//! the live example: it measures a frozen commit.
//!
...
fn mix_chain<C: Context, const W: usize, const Q: usize>(
    ...
    let (mut input, strip_prod) = timed(|| {
        ballots_prod
            .into_par_iter()
            .map(|c| setup.ny_pk.strip(c, ENC_CTX).unwrap())
            .collect::<Vec<_>>()
    });
    let (mut input_ver, strip_ver) = timed(|| {
        ballots_ver
            .into_par_iter()
            .map(|c| setup.ny_pk.strip(c, ENC_CTX).unwrap())
            .collect::<Vec<_>>()
