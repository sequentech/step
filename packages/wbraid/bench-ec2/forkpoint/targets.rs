...
//! Top-level target snapshot benchmark — **fork-point build**.
//!
//! A frozen copy of `crates/vsc/examples/targets.rs` as it stood at the
//! milestone `185dbbede2`, which used only public APIs present at the fork
//! point `657cb05c20`: `bench-ec2/remote-bench.sh` copies this directory into
//! a baseline that predates `strip_all` (`BASE_EXAMPLES_DIR`). Same five
//! targets, same CSV; strip is the per-item `strip` in parallel. Never edit
//! it to track the live example: it measures a frozen commit.
//!
