        ...
        if let Some(db) = self.db.borrow().as_ref() {
            ...
            let onerror = Closure::once(move |event: web_sys::Event| {
                if let Some(tx) = error_sender.borrow_mut().take() {
                    let _ = tx.send(Err(anyhow::anyhow!("Failed to delete metadata: {:?}", event)));
                }
            });
    ...
    fn verify_and_store<C: Context>(
        ...
        for (idx, msg) in new_messages.iter().enumerate() {
            ...
            web_sys::console::log_1(&JsValue::from_str(&format!(
                ...
                idx, persistent.hash_list.len(), new_local_id, msg.id
            )));
        ...
        if new_count > 0 {
            web_sys::console::log_1(&JsValue::from_str(&format!(
                "✅ Stored {} new messages (store now has {} total)", new_count, persistent.hash_list.len()
            )));
        }
