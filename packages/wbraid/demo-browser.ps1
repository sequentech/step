# Demo script for testing browser-based trustees with B4/braid
# Starts B4 + (N-1) native trustees, leaving one slot for a browser trustee
#
# PREREQUISITES:
#   1. LocalStack must be running: .\localstack.ps1
#   2. Build WASM first: .\build-wasm.ps1
#   3. Then run this script: .\demo-browser.ps1
#   4. In a SEPARATE terminal, launch the web server so the browser trustee
#      page is reachable: .\serve.ps1 (serves http://127.0.0.1:8080)
...
# Single source of truth for the demo database. Passed explicitly to every
# demo_tool invocation and to the server window so that an inherited
# DATABASE_URL in the caller's shell cannot make them diverge.
$DbUrl = 'sqlite:b4.db?mode=rwc'
$DbFile = 'b4.db'
...
if (-not (Test-Path ".\crates\braid\pkg\braid.js")) {
    Write-Error "WASM build not found!"
    Write-Host ""
    Write-Host "Please build WASM first:" -ForegroundColor Yellow
    Write-Host "  .\build-wasm.ps1" -ForegroundColor White
    Write-Host ""
    exit 1
}
...
function Cleanup {
    ...
    Get-Process -ErrorAction SilentlyContinue | Where-Object { 
        $_.ProcessName -match '^(cargo|b4|main|main_concurrent|demo_tool|python)$' 
    } | ForEach-Object {
    ...
    if (Test-Path ".\$DbFile") { Remove-Item -Path ".\$DbFile" -Force }
    if (Test-Path ".\$DbFile-shm") { Remove-Item -Path ".\$DbFile-shm" -Force }
    if (Test-Path ".\$DbFile-wal") { Remove-Item -Path ".\$DbFile-wal" -Force }
    if (Test-Path ".\demo") { 
...
Write-Host ""
Write-Host "NOTE: This script does NOT start the web server. In a SEPARATE terminal" -ForegroundColor Yellow
Write-Host "      run '.\serve.ps1' to launch the Python HTTP server, otherwise the" -ForegroundColor Yellow
Write-Host "      browser trustee page below will not load." -ForegroundColor Yellow
Write-Host ""
Write-Host "1. Start the web server (separate terminal):  .\serve.ps1" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  - Web Server:       Start separately with .\serve.ps1 (port 8080)" -ForegroundColor Gray
...
Write-Host ""
Write-Host "B4 Protocol is running! Watch the trustee windows and browser console..." -ForegroundColor Green
