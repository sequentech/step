...
/// due to a duplicate fingerprint.
...
/// Deletes the certificate authorities matching the given ids.
...
/// If `ids` is empty, returns PEMs for all CAs (same behaviour as
/// `get_certificate_authorities_pem`).
#[instrument(skip(transaction), err)]
pub async fn get_certificate_authorities_pem_by_ids(
    ...
    if ids.is_empty() {
        return get_certificate_authorities_pem(transaction).await;
    }
