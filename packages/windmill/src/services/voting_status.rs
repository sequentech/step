...
pub struct ElectionStatusInfo {
    pub is_not_started_votes: i64,
    pub is_open_votes: i64,
    pub is_closed_votes: i64,
    pub is_started_votes: i64,
    pub is_kiosk_closed_votes: i64,
}

#[instrument(skip(election), ret)]
pub fn get_election_status_info(election: &Election) -> ElectionStatusInfo {
    let mut is_not_started_votes: i64 = 0;
    let mut is_open_votes: i64 = 0;
    let mut is_closed_votes: i64 = 0;
    let mut is_started_votes: i64 = 0;
    let mut is_kiosk_closed_votes: i64 = 0;
    ...
    match status.clone() {
        Some(status) => {
            match status.voting_status {
                ...
                // opened votes
                VotingStatus::NOT_STARTED => is_not_started_votes = 1,
                ...
                VotingStatus::PAUSED => is_started_votes = 1,
                // If voting is closed:
                VotingStatus::CLOSED => {
            ...
            match status.kiosk_voting_status {
                VotingStatus::CLOSED => {
                    is_kiosk_closed_votes = 1;
                }
                _ => {}
            }
        }
    ...
    ElectionStatusInfo {
        is_not_started_votes,
        is_open_votes,
        is_closed_votes,
        is_started_votes,
        is_kiosk_closed_votes,
    }
}
