...
pub async fn get_upload_url(
    ...
    let path = match is_public {
        ...
        false => s3::get_document_key(&tenant_id.to_string(), None, &document.id, &name.to_string()),
    };
...
pub async fn get_document_as_temp_file(
    ...
    let document_s3_key = s3::get_document_key(tenant_id, None, &document.id, &document_name);
