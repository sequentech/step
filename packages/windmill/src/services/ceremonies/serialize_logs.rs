...
#[instrument(skip(current_logs))]
pub fn append_tally_finished(current_logs: &[Log], election_ids: &Vec<String>) -> Vec<Log> {
    let mut logs: Vec<Log> = current_logs.to_owned();
    logs.push(Log {
        created_date: ISO8601::to_string(&ISO8601::now()),
...
#[instrument(skip(current_logs))]
pub fn append_tally_updated(current_logs: &[Log], election_ids: &Vec<String>) -> Vec<Log> {
    let mut logs: Vec<Log> = current_logs.to_owned();
    logs.push(Log {
        created_date: ISO8601::to_string(&ISO8601::now()),
...
#[instrument(skip(current_logs))]
pub fn append_tally_recount_log(current_logs: &[Log], election_ids: &Vec<String>) -> Vec<Log> {
    let mut logs: Vec<Log> = current_logs.to_owned();
    logs.push(Log {
        created_date: ISO8601::to_string(&ISO8601::now()),
