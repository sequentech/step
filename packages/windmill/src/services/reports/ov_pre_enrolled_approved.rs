...
impl PreEnrolledVoterTemplate {
    ...
    async fn prepare_user_data_common(&self) -> Result<UserData> {
        ...
        let report_hash = get_report_hash(&ReportType::OV_PRE_ENROLLED_APPROVED.to_string())
            .await
            .unwrap_or("-".to_string());
    ...
    async fn prepare_data_batch(
        ...
        let area_final_data = UserData {
            areas: vec![UserDataArea {
                ...
                number_of_ovs_approved_by_sbei: total_sbei_approved,
                number_of_ovs_approved_by_ofov: total_ofov_approved,
    ...
    fn zip_filename(&self) -> String {
        format!(
            "ov_pre_enrolled_approved_{}_{}_final.zip",
            self.ids.election_event_id,
...
impl TemplateRenderer for PreEnrolledVoterTemplate {
    ...
    fn base_name(&self) -> String {
        "ov_pre_enrolled_approved".to_string()
    }

    fn prefix(&self) -> String {
        format!(
            "ov_pre_enrolled_approved_{}_{}_{}",
            self.ids.tenant_id,
            self.ids.election_event_id,
            self.ids.election_id.clone().unwrap_or_default()
        )
                ...
                for election in elections {
                    ...
                    for area in election_areas.iter() {
                        areas.push(UserDataArea {
                            ...
                            voted: 0,
                            not_voted: 0,
                            number_of_ovs_approved_by_system: 0,
                            number_of_ovs_approved_by_sbei: 0,
                            number_of_ovs_approved_by_ofov: 0,
                            total: 0,
                        })
                    }
                }
