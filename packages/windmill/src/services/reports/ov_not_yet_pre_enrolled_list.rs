...
use crate::services::reports::voters::get_voters_data;
use crate::services::reports::voters::{EnrollmentFilters, FilterListVoters};
...
use sequent_core::{
    ...
    types::templates::{EmailConfig, ReportExtraConfig},
};
...
impl NotPreEnrolledListTemplate {
    ...
    async fn prepare_user_data_common(&self) -> Result<UserData> {
        ...
        let report_hash = get_report_hash(&ReportType::OV_NOT_YET_PRE_ENROLLED_LIST.to_string())
            .await
            .unwrap_or("-".to_string());
    ...
    async fn generate_report_area(
        ...
        info!("inside generate_report_area for area {}", area.area_name);
    ...
    fn zip_filename(&self) -> String {
        format!(
            "ov_not_yet_pre_enrolled_list_{}_{}_final.zip",
            self.ids.election_event_id,
