...
// Extend the UserDataArea struct to include the IDs needed for batching.
...
impl OVUsersWhoVotedTemplate {
    ...
    fn zip_filename(&self) -> String {
        format!(
            "ov_who_voted_{}_{}_final.zip",
            self.ids.election_event_id,
            self.ids.election_id.clone().unwrap_or_default()
        )
    }

    /// Prepares the common user data: execution annotations and an empty areas vector.
    #[instrument(err)]
    ...
    /// Batches the user data for a given area.
    ...
    async fn prepare_data_batch(
        ...
        let area_final_data = UserData {
            areas: vec![UserDataArea {
                ...
                voting_privilege_voted: 0, // TODO: update when real data available
    ...
    /// Generates PDF batches for a given area and writes them to the reports folder.
    #[instrument(err, skip_all)]
    async fn generate_report_area(
        ...
        loop {
            ...
            debug!("User data map in template renderer: {user_data_map:#?}");
            ...
            let system_template = self
                .get_system_template()
                .await
                .map_err(|e| anyhow!("Error getting default system template: {e:?}"))?;
...
impl TemplateRenderer for OVUsersWhoVotedTemplate {
    ...
    /// – Retrieves the template and extra config,
    /// – Rebuilds the areas data,
    /// – Processes report batches concurrently,
    /// – Compresses the results into a zip,
    /// – Uploads the file and sends email notifications.
    #[instrument(err)]
    async fn execute_report_inner(
        ...
        let encrypted_temp_data: Option<TempPath> = if let Some(report) = &report {
            if report.encryption_policy == EReportEncryption::ConfiguredPassword {
                ...
                crate::services::consolidation::aes_256_cbc_encrypt::encrypt_file_aes_256_cbc(
                    &final_file_path,
                    &encrypted_temp_path,
                    &encryption_password,
                )
        ...
        if let Some(enc_temp_path) = encrypted_temp_data {
            ...
            if self.should_send_email(is_scheduled_task) {
                ...
                let email_sender = crate::services::providers::email_sender::EmailSender::new()
                    .await
                ...
                email_sender
                    .send(
                        ...
                        vec![crate::services::providers::email_sender::Attachment {
            ...
            if self.should_send_email(is_scheduled_task) {
                ...
                let email_sender = crate::services::providers::email_sender::EmailSender::new()
                    .await
                    .map_err(|e| anyhow!(format!("Error getting email sender {e:?}")))?;
                let final_file_bytes = fs::read(&final_file_path)
