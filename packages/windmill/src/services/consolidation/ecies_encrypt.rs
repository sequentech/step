...
pub fn ecies_encrypt_string0(public_key_pem: &str, password: &[u8]) -> Result<String> {
    // Parse the PEM file and extract the EC key
    let ec_key = EcKey::public_key_from_pem(public_key_pem.as_bytes())
    .context("Failed to parse PEM and extract EC key")?;

    // Create a BigNumContext
    let mut ctx = BigNumContext::new()
        .context("Failed to create BigNumContext")?;

    // Extract the raw public key bytes
    let public_key_bytes = ec_key.public_key().to_bytes(&ec_key.group(), openssl::ec::PointConversionForm::UNCOMPRESSED, &mut ctx)
        .context("Failed to extract raw public key bytes")?;

    // Encrypt the password
    let encrypted_data = encrypt(&public_key_bytes, password)?;
...
pub fn ecies_encrypt_string(
    ...
    let public_pkey = PKey::from_ec_key(public_key)
        .context("Failed to convert public key to PKey")?;

    // Derive the shared secret using ECDH
    let shared_secret = ephemeral_pkey.derive(&public_pkey)
    ...
    let mut count = crypter.update(password, &mut ciphertext)
...
pub fn ecies_sign_data(public_key_pem_str: &str, acm_key_pair: &EciesKeyPair,  data: &[u8]) -> Result<(String, String)> {
