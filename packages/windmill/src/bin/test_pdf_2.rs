...
#[structopt(
    name = "test-pdf-2",
    about = "Test parallel PDF rendering with different configurations",
...
async fn generate_pdf(html: String, pdf_options: Option<headless_chrome::types::PrintToPdfOptions>, name: &str) -> Result<()> {
    ...
    println!("{}: PDF total length: {} bytes", name, pdf_bytes.len());

    // Check if it starts with PDF magic number
    if pdf_bytes.starts_with(b"%PDF") {
        println!("{}: Valid PDF magic number detected", name);
    } else {
        println!("{}: WARNING: PDF magic number not found!", name);
        println!("{}: First 100 bytes as string: {}", name, String::from_utf8_lossy(&pdf_bytes[..100.min(pdf_bytes.len())]));
    }
    ...
    let output_path = output_dir.join(format!("{}_{}.pdf", name, timestamp));

    fs::write(&output_path, pdf_bytes)?;
    println!("{}: PDF written to {}", name, output_path.display());
...
async fn main() -> Result<()> {
    ...
    // Complex HTML input (same as test_pdf_1)
    ...
    // Portrait configuration (A4 size in inches)
    let portrait_options = headless_chrome::types::PrintToPdfOptions {
        landscape: Some(false),
        scale: Some(1.0),
        paper_width: Some(8.5),    // Letter width: 8.5"
        paper_height: Some(11.0),  // Letter height: 11"
        margin_top: Some(0.5),
    ...
    // Landscape configuration (same dimensions but swapped)
    let landscape_options = headless_chrome::types::PrintToPdfOptions {
        landscape: Some(true),
        scale: Some(1.0),
        paper_width: Some(8.5),    // Keep original width
        paper_height: Some(11.0),  // Keep original height
        margin_top: Some(0.5),
        margin_bottom: Some(0.5),
        margin_left: Some(0.75),
        margin_right: Some(0.75),
    ...
    let tasks = vec![
        generate_pdf(html.clone(), Some(portrait_options), "portrait"),
        generate_pdf(html.clone(), Some(landscape_options), "landscape"),
    ];
