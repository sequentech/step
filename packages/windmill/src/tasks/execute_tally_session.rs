...
async fn process_plaintexts(
    ...
    event!(Level::WARN, "Num sequent_backend_tally_session_contest = {}", tally_session_data.sequent_backend_tally_session_contest.len());
    ...
    let found_area_contests: HashSet<(String, String)> = almost_vec
        ...
        .collect();
    event!(Level::WARN, "Num found_area_contests = {}", found_area_contests.len());

    let filtered_area_contests: Vec<AreaContestDataType> = almost_vec
        .clone()
        .into_iter()
        .filter(|area_contest| {
            event!(Level::WARN, "areas_tree {:?}", areas_tree);
        ...
        .collect();
    event!(Level::WARN, "Num filtered_area_contests = {}", filtered_area_contests.len());
...
#[instrument(skip(tally_sheet_rows), err)]
