...
pub struct LogEventInput {
    ...
    /// Whether this is an internal event or a Keycloak-typed admin event.
        ...
        loop {
            ...
            for delivery in &batch_deliveries {
                ...
                if let serde_json::Value::Array(arr) = v {
                    if arr.len() < 2 {
                        return Err(
                            "Invalid message format: expected array with at least 2 elements"
                                .into(),
                        );
                    }
                    let payload = &arr[1];
