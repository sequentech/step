...
#[instrument(err)]
async fn manage_election_event_lockdown_start_wrapped(
    ...
    if let Some(election_event_presentation) = election_event.presentation {
        let election_event_presentation: ElectionEventPresentation = ElectionEventPresentation {
            locked_down: LockedDown::LOCKED_DOWN,
...
pub async fn manage_election_event_lockdown_start(
    tenant_id: String,
    election_event_id: String,
    scheduled_event_id: String,
    election_id: String,
    ...
    let lock: PgLock = PgLock::acquire(
        format!(
            "execute_manage_election_event_lockdown_start-{}-{}-{}-{}",
    ...
    let res = provide_hasura_transaction(|hasura_transaction| {
        ...
        Box::pin(async move {
            ...
            manage_election_event_lockdown_start_wrapped(
                hasura_transaction,
                tenant_id,
                election_event_id,
                scheduled_event_id,
