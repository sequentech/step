...
/// One runtime for every test. A pooled connection is driven by the runtime
/// that opened it, and the pool hands it to whichever test asks next: with a
/// runtime per test, a connection dies when the test that opened it ends,
/// even while another test is using it.
...
#[test]
fn merge_task_execution_annotations_merges_into_the_stored_annotations() {
    run(async {
...
#[test]
fn merge_task_execution_annotations_creates_annotations_when_none_are_stored() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn merge_task_execution_annotations_fails_for_a_task_of_another_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn merge_task_execution_annotations_rejects_an_invalid_task_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_task_by_id_with_transaction_returns_the_task_with_its_columns() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_task_by_id_with_transaction_fails_for_a_task_of_another_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn lock_export_task_with_transaction_makes_another_attempt_at_the_task_wait() {
    run(async {
...
#[test]
fn get_tasks_by_election_event_id_returns_the_event_tasks_of_the_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tasks_by_election_event_id_calls_an_invalid_event_id_a_task_uuid() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_scheduled_event_returns_and_stores_the_schedule() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_scheduled_event_rejects_an_invalid_election_event_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_new_scheduled_event_keeps_every_supplied_column() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_new_scheduled_event_accepts_a_schedule_without_tenant_or_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_all_active_events_returns_every_unstopped_schedule_of_any_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_id_maps_the_schedule_columns() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_id_skips_stopped_and_archived_schedules() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_id_without_tenant_or_event_matches_any_scope() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_id_is_none_for_a_schedule_of_another_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_id_fails_on_an_unknown_event_processor() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_task_id_returns_a_stopped_but_not_an_archived_schedule() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let stopped = scheduled_event::find_scheduled_event_by_task_id(
            &tx,
            &w.tenant,
            &w.event,
            "stopped-task",
        ...
        .await
        .unwrap();
        let archived = scheduled_event::find_scheduled_event_by_task_id(
            &tx,
            &w.tenant,
            &w.event,
            "archived-task",
...
#[test]
fn find_scheduled_event_by_task_id_only_looks_in_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn stop_scheduled_event_stops_only_the_target_schedule() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn stop_scheduled_event_keeps_the_first_stop_time() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn stop_scheduled_event_leaves_a_schedule_of_another_tenant_running() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn archive_scheduled_event_stops_and_archives_only_the_target_schedule() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn archive_scheduled_event_overwrites_an_earlier_stop_time() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_scheduled_event_replaces_the_cron_config_and_keeps_the_payload() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_scheduled_event_adds_the_voting_channels_to_the_payload() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_scheduled_event_leaves_a_stopped_schedule_unchanged() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_election_event_id_returns_the_unarchived_event_schedules() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn find_scheduled_event_by_election_event_id_and_event_processor_never_binds_the_processor() {
    run(async {
...
#[test]
fn insert_reports_stores_every_report_with_its_columns() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_reports_files_reports_under_the_given_tenant_and_event() {
    run(async {
...
#[test]
fn insert_reports_reads_back_a_missing_cron_config_as_the_default() {
    run(async {
...
#[test]
fn get_report_by_id_is_none_for_a_report_of_another_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_report_by_id_reads_an_unreadable_cron_config_as_the_default() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_report_by_id_fails_on_an_unknown_encryption_policy() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_all_active_reports_returns_the_reports_with_an_active_cron_config() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_report_last_document_time_records_the_transaction_time_in_utc() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_report_last_document_time_creates_a_cron_config_when_none_is_stored() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_report_last_document_time_fails_for_a_report_of_another_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_template_alias_for_report_returns_the_alias_of_the_election_report() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_template_alias_for_report_falls_back_to_a_report_of_another_election() {
    run(async {
...
#[test]
fn get_template_alias_for_report_is_none_without_a_report_of_the_type_in_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_reports_by_election_event_id_returns_the_event_reports_of_the_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_reports_by_election_id_returns_the_election_reports_of_the_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_reports_by_election_id_rejects_an_invalid_election_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_report_by_type_only_returns_a_report_of_the_requested_election() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_report_by_type_drops_the_election_filter_for_an_invalid_election_id() {
    run(async {
...
#[test]
fn insert_document_returns_and_stores_the_document() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_document_without_an_id_stores_a_new_v4_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_document_with_annotations_stores_the_access_annotations() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_document_rejects_an_invalid_document_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_document_returns_the_document_of_the_tenant_and_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_document_is_none_outside_the_tenant_or_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let in_other_event =
            document::get_document(&tx, &w.tenant, Some(w.event.clone()), &w.id(10))
                .await
                .unwrap();
...
#[test]
fn get_document_without_an_event_matches_any_event_of_the_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn delete_documents_deletes_only_the_listed_documents_of_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn delete_documents_with_no_ids_deletes_nothing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn delete_documents_rejects_an_invalid_id_before_deleting() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_exportable_document_ids_leaves_out_voter_secret_documents() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        let voter_secrets =
            serde_json::to_value(DocumentAnnotations::voter_secret_export()).unwrap();
        let password =
...
#[test]
fn get_exportable_document_ids_includes_voter_secret_documents_when_allowed() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_exportable_document_ids_fails_on_access_annotations_it_cannot_read() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_support_material_documents_pairs_visible_materials_with_their_documents() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_support_material_documents_is_an_empty_list_without_materials() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn upsert_lock_acquires_a_free_key() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
...
#[test]
fn upsert_lock_refuses_a_key_another_holder_keeps_alive() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let key = format!("{}-lock", ids!().id(10));
...
#[test]
fn upsert_lock_takes_over_an_expired_lock() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let key = format!("{}-lock", ids!().id(10));
...
#[test]
fn upsert_lock_renews_a_lock_for_its_holder() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let key = format!("{}-lock", ids!().id(10));
...
#[test]
fn delete_lock_only_releases_a_lock_for_its_holder() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let key = format!("{}-lock", ids!().id(10));
...
#[test]
fn render_report_task_fails_for_an_unknown_tenant_before_writing_a_document() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let ids = ids!();
...
#[test]
fn render_report_task_fails_on_an_invalid_template_before_writing_a_document() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
