...
#[test]
fn insert_tally_sheet_returns_and_stores_the_sheet() {
    run(async {
...
#[test]
fn insert_tally_sheet_links_the_sheet_to_its_import() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_sheet_rejects_a_second_live_sheet_of_the_same_version() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_sheet_rejects_an_invalid_area_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_approved_tally_sheets_by_event_needs_approval_review_and_no_deletion() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_approved_tally_sheets_by_event_maps_the_reviewed_sheet() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_latest_approved_tally_sheet_returns_the_highest_live_approved_version() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_latest_approved_tally_sheet_is_none_without_an_approved_sheet_of_the_ballot_box() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_latest_ballot_box_version_is_zero_for_an_empty_ballot_box() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_latest_ballot_box_version_counts_deleted_versions() {
    run(async {
...
#[test]
fn get_latest_ballot_box_tally_sheet_ignores_deleted_versions() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_latest_ballot_box_tally_sheet_fails_on_an_unknown_status() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_latest_ballot_box_tally_sheet_fails_on_content_it_cannot_read() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn lock_ballot_box_version_assignment_makes_another_assignment_to_the_ballot_box_wait() {
    run(async {
...
#[test]
fn lock_ballot_box_version_assignment_keys_the_lock_on_the_ids_as_written() {
    run(async {
...
#[test]
fn soft_delete_tally_sheet_leftover_versions_deletes_the_other_live_versions_of_the_box() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn soft_delete_tally_sheet_leftover_versions_needs_the_sheet_channel() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn review_tally_sheet_status_records_the_review_of_a_pending_sheet() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn review_tally_sheet_status_leaves_an_already_reviewed_sheet_unchanged() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn review_tally_sheet_status_reports_a_sheet_of_another_event_as_not_found() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn review_tally_sheet_status_reports_a_deleted_sheet_as_not_found() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn review_tally_sheet_status_rejects_an_invalid_tally_sheet_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_sheet_import_returns_and_stores_the_import() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_sheet_import_without_optional_values_stores_nulls() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_by_id_returns_the_import_of_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_by_id_is_none_for_an_import_of_another_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_by_id_fails_on_an_unknown_status() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_by_id_cannot_read_the_default_empty_summary() {
    run(async {
...
#[test]
fn insert_tally_sheet_import_items_stores_every_item_with_its_columns() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_sheet_import_items_with_no_items_writes_nothing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_sheet_import_items_rejects_an_invalid_generated_sheet_id_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_items_for_review_orders_the_import_items_by_ballot_box() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_items_for_review_maps_the_baseline_of_an_item() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sheet_import_items_for_review_fails_on_an_unknown_channel() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_sheet_import_status_returns_the_updated_import() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_sheet_import_status_fails_for_an_import_of_another_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_sheet_import_items_status_updates_every_item_of_the_import_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_sheet_import_status_with_conflict_count_records_the_count_in_the_summary() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_sheet_import_items_status_by_ids_updates_only_the_listed_items_of_the_import() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_sheet_import_items_status_by_ids_without_ids_changes_nothing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
