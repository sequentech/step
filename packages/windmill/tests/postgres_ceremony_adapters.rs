...
#[test]
fn insert_keys_ceremony_returns_and_stores_every_supplied_column() {
    run(async {
...
#[test]
fn insert_keys_ceremony_rejects_a_trustee_id_that_is_not_a_v4_uuid_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_keys_ceremonies_returns_the_ceremonies_of_the_event_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_keys_ceremony_by_id_maps_null_optional_columns_to_none() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_keys_ceremony_by_id_reports_a_ceremony_of_another_event_as_not_found() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_keys_ceremony_by_id_rejects_a_uuid_that_is_not_version_4() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn lock_keys_ceremony_by_id_returns_the_ceremony_it_locks() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn lock_keys_ceremony_by_id_makes_another_transaction_wait_until_it_ends() {
    run(async {
        ...
        waiting
            ...
            .unwrap();
        let error =
            keys_ceremony::lock_keys_ceremony_by_id(&waiting, &tenant_id, &event_id, &ceremony)
                .await
                .unwrap_err();
...
#[test]
fn update_keys_ceremony_status_changes_only_the_ceremony_of_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        keys_ceremony_row(&tx, &w.tenant, &w.other_event, &w.id(10)).await;
        let status =
            json!({"stop_date": "2026-01-01", "public_key": "pk", "logs": [], "trustees": []});
...
#[test]
fn update_keys_ceremony_status_fails_for_a_ceremony_of_another_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn list_keys_ceremony_without_permission_labels_returns_every_ceremony_of_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn list_keys_ceremony_returns_the_ceremonies_sharing_a_permission_label() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_trustees_by_id_returns_the_requested_trustees_of_the_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let trustees =
            trustee::get_trustees_by_id(&tx, &w.tenant, &vec![w.id(10), w.id(12), w.id(13)])
                .await
                .unwrap();
...
#[test]
fn get_trustees_by_id_rejects_an_invalid_trustee_id() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_trustees_by_name_returns_the_named_trustees_of_the_tenant() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_trustee_by_name_returns_the_trustee_with_its_columns() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_trustee_by_name_reports_a_trustee_of_another_tenant_as_not_found() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_all_trustees_returns_every_trustee_of_the_tenant_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_returns_and_stores_the_session() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_without_configuration_stores_null() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_rejects_an_invalid_election_id_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn pending_post_tally_task_sessions_are_those_marked_incomplete_newest_first() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let pending =
            tally_session::get_tally_session_by_election_event_id_pending_post_tally_task(
                &tx, &w.tenant, &w.event,
            )
            .await
            .unwrap();
...
#[test]
fn get_tally_sessions_by_election_event_id_returns_every_session_newest_first() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sessions_by_election_event_id_only_active_keeps_unfinished_sessions_in_progress() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_by_id_maps_null_optional_columns_to_none() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_by_id_reports_a_session_of_another_event_as_not_found() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_by_id_fails_on_a_configuration_it_cannot_read() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_by_id_rejects_an_invalid_uuid() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn lock_tally_session_for_update_reports_a_missing_session() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn lock_tally_session_for_update_makes_another_transaction_wait_until_it_ends() {
    run(async {
        // Another transaction only sees committed rows, so this setup commits and
        // deletes its rows at the end.
        let pool = schema::pool().await;
...
#[test]
fn update_tally_session_annotation_replaces_the_annotations_of_the_event_session_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_sessions_by_election_id_returns_the_sessions_tallying_the_election() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_session_status_sets_status_and_completion_of_the_event_session_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn set_post_tally_task_completed_adds_the_flag_to_existing_annotations() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn set_post_tally_task_completed_leaves_null_annotations_null() {
    run(async {
...
#[test]
fn set_tally_session_completed_records_completion_and_a_pending_post_task() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn set_tally_session_completed_without_annotations_leaves_no_pending_post_task() {
    run(async {
...
#[test]
fn insert_many_tally_sessions_stores_every_session_with_its_columns() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_sessions_stores_missing_election_and_area_ids_as_empty_arrays() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_sessions_with_no_sessions_returns_nothing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
...
#[test]
fn insert_many_tally_sessions_rejects_an_invalid_area_id_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_contest_returns_and_stores_the_contest_batch() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_contest_without_a_contest_stores_a_null_contest() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_contest_wraps_a_batch_number_beyond_the_int4_column() {
    run(async {
...
#[test]
fn insert_tally_session_contest_rejects_an_invalid_contest_id_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_highest_batch_is_zero_for_an_event_without_contests() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let batch =
            tally_session_contest::get_tally_session_highest_batch(&tx, &w.tenant, &w.event)
                .await
                .unwrap();
...
#[test]
fn get_tally_session_highest_batch_skips_the_weight_batches_of_the_last_contest_area() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_contests_returns_the_contests_of_the_session_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let contests =
            tally_session_contest::get_tally_session_contests(&tx, &w.tenant, &w.event, &d)
                .await
                .unwrap();
...
#[test]
fn get_event_tally_session_contest_returns_every_contest_of_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let contests =
            tally_session_contest::get_event_tally_session_contest(&tx, &w.tenant, &w.event)
                .await
                .unwrap();
...
#[test]
fn update_tally_session_contests_annotations_replaces_the_annotations_of_listed_contests() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn update_tally_session_contests_annotations_skips_contests_without_a_matching_row() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_session_contests_keeps_the_supplied_ids_and_timestamps() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let inserted =
            tally_session_contest::insert_many_tally_session_contests(&tx, contests.clone())
                .await
                .unwrap();
...
#[test]
fn insert_many_tally_session_contests_stores_missing_timestamps_as_null() {
    run(async {
...
#[test]
fn insert_many_tally_session_contests_with_no_contests_returns_nothing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
...
#[test]
fn insert_many_tally_session_contests_rejects_an_invalid_area_id_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_execution_returns_and_stores_status_documents_and_reason() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_execution_dates_each_execution_when_it_is_written() {
    run(async {
...
#[test]
fn insert_tally_session_execution_without_optional_values_stores_nulls() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_tally_session_execution_rejects_an_invalid_results_event_id_before_writing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_executions_returns_the_executions_of_the_session_newest_first() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
        ...
        let executions = tally_session_execution::get_tally_session_executions(
...
#[test]
fn get_last_tally_session_execution_returns_the_newest_execution_of_the_session() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_last_tally_session_execution_is_none_without_executions() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_last_tally_session_execution_prefers_an_execution_without_created_at() {
    run(async {
...
#[test]
fn get_event_tally_session_executions_returns_every_execution_of_the_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_execution_documents_returns_the_stored_documents() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_execution_documents_is_none_when_no_documents_were_stored() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_execution_documents_is_none_for_an_execution_of_another_event() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn get_tally_session_execution_documents_fails_on_documents_it_cannot_read() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_session_executions_keeps_the_supplied_ids_timestamps_and_status() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_session_executions_does_not_store_documents_or_run_reason() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_session_executions_stores_a_missing_created_at_as_null() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
...
#[test]
fn insert_many_tally_session_executions_with_no_executions_returns_nothing() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
...
#[test]
fn update_tally_session_execution_documents_replaces_the_documents_of_the_target_only() {
    run(async {
        let mut client = schema::pool().await.get().await.unwrap();
        let tx = client.transaction().await.unwrap();
        let w = World::new(&tx, ids!()).await;
