...
beforeAll(async () => {
    ...
    const initialize =
        jest.requireActual<(options?: object) => Promise<SqlJsStatic>>("sql.js/dist/sql-asm.js")
    sql = await initialize()
})
afterEach(() => {
    jest.restoreAllMocks()
    jest.mocked(initSqlJs).mockReset()
})
...
it("queries literal SQLite rows with bound parameters and reads available dataset tables", () => {
    ...
    try {
        db.run(
            "CREATE TABLE election (id TEXT, name TEXT); INSERT INTO election VALUES ('e1', 'First'), ('e2', 'Second')"
        )
        expect(queryRows(db, "SELECT id, name FROM election WHERE id = ?", ["e2"])).toEqual([
            {id: "e2", name: "Second"},
        ])
        expect(queryRows(db, "SELECT id FROM election WHERE id = ?", ["e3"])).toEqual([])
        const dataset = readResultsDataset(db)
        expect(dataset.election).toEqual([
            {id: "e1", name: "First"},
            {id: "e2", name: "Second"},
        ])
...
it("frees a prepared statement when iteration or row decoding fails", () => {
    for (const failure of ["step", "getAsObject"] as const) {
        const statement = {
            step: jest.fn(() => true),
            getAsObject: jest.fn(() => ({id: 17})),
            free: jest.fn(),
        }
        statement[failure].mockImplementation(() => {
            throw new Error(`synthetic ${failure} failure`)
        })
        const db = {prepare: jest.fn(() => statement)} as unknown as Database
        expect(() => queryRows(db, "SELECT id FROM fixture")).toThrow(
...
it("loads real exported SQLite bytes with no-store fetch semantics", async () => {
    ...
    const fetch = jest
        ...
        .mockResolvedValue(new Response(Uint8Array.from(bytes).buffer))
    const loaded = await loadSqliteDatabase("https://files.invalid/data.sqlite")
    try {
        expect(queryRows(loaded, "SELECT id FROM election")).toEqual([{id: "synthetic"}])
    } finally {
        loaded.close()
    }
...
it("rejects failed downloads and reports corrupt SQLite at the query boundary", async () => {
    ...
    await expect(loadSqliteDatabase("https://files.invalid/missing")).rejects.toThrow(
        "Unable to load results artifact: HTTP 404"
    )
    ...
    try {
        expect(() => queryRows(corrupt, "SELECT name FROM sqlite_master")).toThrow(
