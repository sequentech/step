// src/braid/TrusteeDashboard.tsx
...
export const TrusteeDashboard = () => {
  ...
  const progress = state.max_messages ? Math.round((state.current_messages || 0) / state.max_messages * 100) : 0
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={12}>
                  <TextField
                    ...
                    onChange={e => setConfig({ ...config, signing_key_sk: e.target.value })}
                  />
                </Grid>
                <Grid size={12}>
                  <TextField
                    ...
                    onChange={e => setConfig({ ...config, signing_key_pk: e.target.value })}
                  />
                </Grid>
                <Grid size={12}>
                  <TextField
                ...
                <Grid size={12}>
                  <TextField
                    ...
                    fullWidth
                    value={config.b4_url}
                    onChange={e => setConfig({ ...config, b4_url: e.target.value })}
                  />
                </Grid>
                <Grid size={12}>
                  <Button
                ...
                <Grid size={4}>
                  <Paper variant="outlined" sx={{ p: 2 }}>
                    ...
                    <Typography variant="h5">
                      {state.current_messages || 0} / {state.max_messages || 0}
                    </Typography>
                  </Paper>
                </Grid>
                <Grid size={4}>
                  <Paper variant="outlined" sx={{ p: 2 }}>
                    ...
                    <Typography variant="body2" noWrap>{lastAction}</Typography>
                  </Paper>
                </Grid>
                <Grid size={4}>
                  <Paper variant="outlined" sx={{ p: 2 }}>
                    <Typography variant="caption">Board</Typography>
                    <Typography variant="body2">{state.board_name || '-'}</Typography>
                  </Paper>
                </Grid>
              </Grid>

              <LinearProgress variant="determinate" value={progress} />
              <Typography variant="body2" align="right" mt={1}>{progress}%</Typography>

              <Collapse in={boardSummary.length > 0}>
                <Box mt={3}>
                  <Typography variant="subtitle2">Local Board ({boardSummary.length})</Typography>
                  <List dense>
                    {Object.entries(
                      boardSummary.reduce<Record<string, any[]>>((acc, s: any) => {
                        ;(acc[s.kind] ??= []).push(s)
                        return acc
                      }, {})
                    ).map(([kind, items]) => (
                      <ListItem key={kind} disablePadding>
                        <ListItemButton>
                          <ListItemText
                            primary={kind}
                            secondary={items
                              .map((s: any) => (s.signer === 4294967295 ? 'PM' : `T${s.signer}`))
                              .join(', ')}
                          />
                        </ListItemButton>
                      </ListItem>
