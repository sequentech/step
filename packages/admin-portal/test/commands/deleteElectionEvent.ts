...
module.exports = {
	...
	deleteElection: (browser) => {
		// delete election
		browser.hoverAndClick(`a[title='${createElectionEvent.config.electionName}'] + div.menu-actions-${electionLink!}`)
		browser.pause(200)
	...
	deleteContest: (browser) => {
		// delete contest
		browser
			.hoverAndClick(`a[title='${createElectionEvent.config.contestName}'] + div.menu-actions-${contestLink!}`)
		browser.pause(200)
	...
	deleteCandidates: async (browser) => {
		...
		browser.hoverAndClick(`a[title='${createElectionEvent.config.candidate2Name}'] + div.menu-actions-${candidateLink!}`)		
		browser.pause(200)
		...
		browser
			.hoverAndClick(`a[title='${createElectionEvent.config.candidate1Name}'] + div.menu-actions-${candidateLink!}`)
