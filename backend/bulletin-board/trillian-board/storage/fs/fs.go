...
func (fs *Storage) Sequence(_ context.Context, leafhash []byte, leaf []byte) (uint64, error) {
	return 0, fmt.Errorf("unimplemented, call SequenceTempFile instead")
}
...
func (fs *Storage) SequenceTempFile(_ context.Context, leafhash []byte, leafTempFilePath string) (uint64, error) {
	// 1. Check for dupe leafhash
	// 2. Hard link temp -> seq file
	...
	for {
		...
		if err := os.Link(leafTempFilePath, seqPath); errors.Is(err, os.ErrExist) {
			// That sequence number is in use, try the next one
			fs.nextSeq++
			continue
		} else if err != nil {
			return 0, fmt.Errorf("failed to link (%s to %s) seq file: %w", leafTempFilePath, seqPath, err)
		}
