...
pub(super) fn compute_plaintexts<C: Ctx>(
    ...
    let m = Message::plaintexts_msg(cfg, *batch, plaintexts, *dfactors_hs, trustee)?;
...
pub(super) fn sign_plaintexts<C: Ctx>(
    ...
    if expected.0 .0 == actual.0 .0 {
        info!("Plaintexts matches expected value");
        let m = Message::plaintexts_signed_msg(cfg, *batch, *plaintexts_h, *dfactors_hs, trustee)?;
        Ok(vec![m])
    } else {
        Err(anyhow!(
...
fn compute_plaintexts_<C: Ctx>(
    ...
    let mut divider = vec![C::E::mul_identity(); ciphertexts.ciphertexts.0.len()];
    ...
    for (t, df_h) in dfactors_hs.0.iter().enumerate() {
        ...
        if t < *threshold {
            ...
            assert_eq!(ciphertexts.ciphertexts.0.len(), dfactors.factors.0.len());
            ...
            let ciphs = ciphertexts.ciphertexts.clone();
            ...
            let it2 = it.zip(ciphs.0.into_par_iter());
            ...
            info!("Processed threshold number of decryption factors");
            break;
        }
    }

    info!("Computing {} plaintexts..", ciphertexts.ciphertexts.0.len());
    let ps = ciphertexts
        .ciphertexts
        .0
        .into_par_iter()
