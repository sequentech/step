...
crepe! {
    ...
    Ballots(cfg_h, batch, ballots_h, pk_h, target_t, trustees) <- InP(p),
    let Predicate::Ballots(cfg_h, batch, ballots_h, pk_h, target_t, trustees) = p;
    ...
    PublicKeySignedAll(cfg_h, pk_h, _shares_h),
    Ballots(cfg_h, batch, _ciphertext_h, pk_h, _signer_t, trustees),
    // Detects that we (self_p) are the trustee assigned to perform the next mix
    Mix(cfg_h, batch, _, ciphertexts_h, mix_number, signer_t, self_p),
    ...
    ConfigurationSignedAll(cfg_h, self_p, _num_t, _threshold),
    PublicKeySignedAll(cfg_h, pk_h, _shares_h),
    Ballots(cfg_h, batch, _source_h, pk_h, _signer_t, trustees),
    ...
    Ballots(cfg_h, batch, _, _pk_h, _, trustees),
